...
Normalize counts and verify correct order
```{r}
vsd <- vst(dds, blind = FALSE)
vst_mat <- assay(vsd)
vst_mat <- t(vst_mat) # Transpose: Samples as rows, Genes as columns

# Ensure the metadata rows exactly match the transformed matrix rows
meta_sub <- as.data.frame(colData(dds))
all(rownames(vst_mat) == rownames(meta_sub)) # Should be TRUE
```

Run PERMANOVA on Condition
```{r}
# 'by = "terms"' tests the significance of the model terms
permanova_result <- adonis2(vst_mat ~ Condition, 
                            data = meta_sub, 
                            method = "euclidean", 
                            permutations = 999)
...
```{r}
# use the vst_mat (samples as rows) and the Condition column
pairwise_results <- pairwise.adonis2(vst_mat ~ Condition, 
                                     data = meta_sub, 
                                     method = "euclidean", 
                                     nperm = 999)
...
Remove TP0 and rerun permanova
```{r}
# Identify samples that are NOT TP0
keep_samples <- rownames(meta_sub)[meta_sub$Timepoint != "TP0"]

# Subset the metadata and the VST matrix
meta_filtered <- meta_sub[keep_samples, ]
vst_filtered <- vst_mat[keep_samples, ]

# Drop the unused 'TP0' level from the factors
meta_filtered$Timepoint <- droplevels(meta_filtered$Timepoint)
meta_filtered$Condition <- droplevels(meta_filtered$Condition)

# Run PERMANOVA with two factors
# Treatment * Timepoint looks at the main effects AND their interaction
res_filtered <- adonis2(vst_filtered ~ Treatment * Timepoint, 
                        data = meta_filtered, 
                        method = "euclidean", 
                        permutations = 999)
...
No significant differences when TP0 is removed. 
