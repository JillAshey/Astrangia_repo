...
png("../output/Molecular/mRNA/enrichment/seasonal_upset_plot_BP.png", width = 8, height = 6, units = "in", res = 600)
upset(upset_data,
      sets = c("February", "June", "August"),
      order.by = "freq",
      sets.bar.color = c("#009E73", "#56B4E9", "#E69F00"), # <-- custom colors
...
Make a custom upset plot that includes custom labels of enriched terms 
```{r}
# Reconstruct data with a factor for sorting
plot_data <- tibble(
  Intersection = c(
    rep("February", 103),
    rep("August", 88),
    rep("August-June", 63),
    rep("June", 49),
    rep("February-June", 11)
  )
) %>%
  # Convert to list format required by ggupset
  mutate(Intersection_List = str_split(Intersection, "-"))

# Labels data (Count_Num for the text, Count_Box for the label position)
labels_data <- tibble(
  Intersection_List = list(
    c("February"), c("August"), c("August", "June"), 
    c("June"), c("February", "June")
  ),
  Count_Num = c(103, 88, 63, 49, 11),
  Label = c(
  ...
  mutate(Text_Pos = Count_Num + 5,    # Position for the raw number
         Box_Pos = Count_Num + 22)   # Position for the box

# Create the Set Size Inset Data
set_size_data <- tibble(
  Month = factor(c("February", "June", "August"), 
                 levels = c("February", "June", "August")),
  Total = c(
    103 + 11, # Feb unique + Feb-June intersection
    49 + 63 + 11, # June unique + both intersections
    88 + 63  # Aug unique + Aug-June intersection
  ),
  Color = c("#009E73", "#E69F00", "#56B4E9")
)

# Build the Inset Plot
inset_plot <- ggplot(set_size_data, aes(y = Month, x = Total, fill = Month)) +
  geom_col(color = "black", fill = "grey80", width = 0.7, show.legend = FALSE) +
  #scale_fill_manual(values = c("February" = "#009E73", "August" = "#E69F00", "June" = "#56B4E9")) +
  geom_text(aes(label = Total), hjust = -0.2, size = 3) +
  scale_x_continuous(expand = expansion(mult = c(0, 0.3))) +
  labs(x = "Total Terms", y = "") +
  theme_classic() +
  theme(
    ...
    plot.background = element_rect(fill = "white", color = "black") # Subtle border
  ); inset_plot



# Plot
seasonal_go <- ggplot(plot_data, aes(x = Intersection_List)) +
  # Bars with black outlines, ordered by size (decreasing)
  geom_bar(fill = "grey50", color = "black", width = 0.6) + 
  
  # UpSet Matrix Configuration
  scale_x_upset(order_by = "freq") + # "freq" sorts high to low automatically
  
  # Add the Count Value (e.g., 103) above the bar
  geom_text(data = labels_data, 
            aes(x = Intersection_List, y = Text_Pos, label = Count_Num),
            size = 4) +
  
  # Add the Term Boxes above the numbers
  geom_label(data = labels_data,
             aes(x = Intersection_List, y = Box_Pos, label = Label),
             size = 4, lineheight = 0.85,
             label.size = 0.4, fill = "white", label.padding = unit(0.3, "lines")) +
  
  # Y-axis with ticks every 25
  scale_y_continuous(breaks = seq(0, 150, by = 25), 
                     limits = c(0, 160), 
                     expand = c(0, 0)) +
  
  labs(x = "", y = "Number of Enriched BP Terms") +
  
  theme_classic() +
  theme(
    axis.title.y = element_text(size = 18),
    axis.text.y = element_text(color = "black", size = 15),
    # Coloring the month labels to match your requested palette
    # February = #009E73, June = #56B4E9, August = #E69F00
    axis.text.x = element_text(color = c("#009E73", "#E69F00", "#56B4E9")),
    plot.margin = margin(t = 20, r = 10, b = 10, l = 10)
  ); seasonal_go

# Combine using patchwork
final_plot <- seasonal_go + 
  inset_element(inset_plot, 
                left = 0.65,   # Start at 65% of width
                bottom = 0.65, # Start at 65% of height
                right = 0.98,  # End near the edge
                top = 0.98,    # End near the top
                align_to = "panel");final_plot

# Save only seasonal go plot without inset 
ggsave("../output/Molecular/mRNA/enrichment/seasonal_upset_plot_labeled_BP.pdf", 
       seasonal_go, width = 12, height = 7, limitsize = FALSE, dpi = 300)
ggsave("../output/Molecular/mRNA/enrichment/seasonal_upset_plot_labeled_BP.png", 
       seasonal_go, width = 12, height = 7, limitsize = FALSE, dpi = 300)

# Save plot with inset 
ggsave("../output/Molecular/mRNA/enrichment/seasonal_upset_plot_with_inset_BP.png", 
       final_plot, width = 12, height = 7, dpi = 300)
```
