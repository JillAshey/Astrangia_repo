...
```{r}
# 1. Reconstruct data with a factor for sorting
plot_data <- tibble(
  Intersection = c(
    rep("February", 103),
    rep("August", 88),
...
# 2. Labels data (Count_Num for the text, Count_Box for the label position)
labels_data <- tibble(
...
# 3. Plot
ggplot(plot_data, aes(x = Intersection_List)) +
  # Bars with black outlines, ordered by size (decreasing)
  geom_bar(fill = "grey90", color = "black", width = 0.6) + 
