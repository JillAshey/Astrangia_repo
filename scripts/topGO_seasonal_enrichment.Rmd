...
# ── 4. Top 5 GO terms per group by p-value (Retaining Enrichment) ────────────
get_top5 <- function(term_list, group_name) {
  ...
  all_data %>%
    ...
    group_by(Term) %>%
    summarise(
      min_pval   = min(Fisher, na.rm = TRUE),
      enrichment = max(Enrichment, na.rm = TRUE), # Retain Fold Enrichment
      .groups    = "drop"
    ) %>%
    arrange(min_pval) %>%
    slice_head(n = 5) %>%
...
# Shared x-axis limits & enrichment range across all panels
x_lim <- c(0, max(top_terms$neg_log_p, na.rm = TRUE) * 1.05)
enrich_limits <- range(top_terms$enrichment, na.rm = TRUE)
...
# ── 8. GO term dot plots (Size mapped to Fold Enrichment) ─────────────────────
...
make_go_panel <- function(grp) {
  ...
  ggplot(d, aes(x = neg_log_p,
    ...
    geom_point(aes(size = enrichment, color = group)) +
    ...
    labs(x = NULL, y = NULL, title = grp, size = "Fold Enrichment") +
    guides(color = "none") + # Explicitly removes the 'group' color legend per panel
...
# Wrap panels and collect only the size legend on the right
...
final_plot <- wrap_plots(left_panel, go_stack, ncol = 2, widths = c(1.8, 2.5)) +
  plot_annotation(
    theme = theme(
...
final_output <- ggdraw(final_plot) +
  ...
  draw_label("B", x = 0.44, y = 0.96, fontface = "bold", size = 40)
