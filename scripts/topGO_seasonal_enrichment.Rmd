...
Subset genes that are downregulated (ie those that were expressed in Feb relative to June and August)
```{r}
feb_june <- feb_june %>%
...
Subset genes that are upregulated relative to February and downregulated relative to August 
```{r}
feb_june <- feb_june %>%
  filter(log2FoldChange > 0)
june_aug <- june_aug %>%
...
Subset genes that are upregulated relative to February and upregulated relative to June 
```{r}
feb_aug <- feb_aug %>%
  filter(log2FoldChange > 0)
june_aug <- june_aug %>%
...
overlapping_terms_feb_june <- intersect(feb_terms, june_terms) # 5 overlapping terms
overlapping_terms_feb_aug <- intersect(feb_terms, aug_terms) # 6 overlapping terms
overlapping_terms_june_aug <- intersect(june_terms, aug_terms) # 39 overlapping terms

common_terms <- Reduce(intersect, list(feb_terms, june_terms, aug_terms))
```

# Intersection of expressed genes with genes targeted by miRNAs
...
## February expressed genes targeted by miRNAs 

Bind with Feb expressed genes 
```{r}
feb_combined <- feb_all_unique %>%
  inner_join(combined_data_pcc, by = c("gene_id" = "mRNA"))
```
