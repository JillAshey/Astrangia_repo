...
light.trt.date
ggsave("output/Light.By.Date.Treatment.pdf", light.trt.date, width = 30, height = 21, units = c("in"))


light.tank.date <- ggplot(light, aes(x=Timepoint, y=PAR, group=Tank)) +
  geom_line(aes(color = Tank), size = 1) 
light.tank.date
ggsave("output/Light.By.Date.Tank.pdf", light.tank.date, width = 30, height = 21, units = c("in"))

plot.all <- grid.arrange(light.trt.date, light.tank.date, ncol=1, nrow=2)
ggsave("output/Light.By.Date.pdf", plot.all, width = 30, height = 25, units = c("in"))
```
