...
# Write to output file
write.csv(pr.out, "output/PR_skeletal_rates.csv")
```
...
skl.plot <- ggplot(pr.save, aes(x = Timepoint, y = micromol.cm2.h, group=Treatment)) +
...
plot <- grid.arrange(skl.plot, clip="off")
ggsave("output/PR/Skeletal.Treatment_PR.pdf", plot, width = 6, height = 5, units = c("in"))
