...
Subset only negative regulation
```{r}
neg_reg <- all_subset %>%
...
```{r}
blast <- read.csv("../output/Molecular/interactions/sigcorr_blast_AST2021.csv")
```
...
```{r}
# Create a binary indicator for presence or absence of pattern
neg_reg$presence <- ifelse(!is.na(neg_reg$pattern), 1, 0)

# Reorder the dataframe based on mirna_FC and mrna_FC
neg_reg <- neg_reg[order(neg_reg$mirna_FC, neg_reg$mrna_FC),]

# Create the heatmap using ggplot2
heatmap <- ggplot(neg_reg, aes(x = function., y = mirna, fill = presence)) +
  geom_tile(color = "white") +
  #scale_fill_manual(values = c("white", "blue"), labels = c("Absent", "Present")) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "mRNA", y = "miRNA", title = "Presence of Pattern in miRNA-mRNA Interactions"); heatmap



presence_matrix <- as.matrix(table(neg_reg$mirna, neg_reg$mrna))
...
heatmap <- pheatmap(
  ...
  color = c("white", "blue"),  # Set color palette
  border_color = "black",  # Remove border color
  fontsize_row = 10,  # Set font size for row labels
  fontsize_col = 10,  # Set font size for column labels
  main = "Presence of Pattern in miRNA-mRNA Interactions",  # Set main title
  angle_col = 45,  # Rotate x-axis labels
  cellwidth = 5,  # Set width of each cell
  cellheight = 5  # Set height of each cell
)
# not showing all of the interactions with the presence matrix or df...
```
