...
The comparisons I can analyze are TP0_v_5_amb, TP0_v_7_amb, TP0_v_5_heat, and TP0_v_7_heat because these were the only comparisons that had differentially expressed miRNAs. The following analysis uses the short stack data. 
...
I will be initially comparing TP0 amb v TP5 amb, TP0 amb v TP7 amb, TP0 amb v TP5 heat, and TP0 amb v TP7 heat -- these were the only comparisons that had both DEGs and DEMs
...
```{r}
TP0_v_5_amb_miRNA_df <- read.csv("../output/Molecular/smRNA/shortstack/TP0_v_5_amb_miRNA.csv") %>%
  dplyr::select(X, log2FoldChange, padj) %>%
  dplyr::rename(gene_id = X, pvalue = padj)
```
...
# Remove bp info 
rownames(mirna_data) <- sub(":.*", "", rownames(mirna_data))
```
...
length(unique(inter0$V1))
length(unique(inter0$V2))
```

This function is calculating the absolute value of the difference between the mRNA and miRNA fold changes for each potential miRNA mRNA combo. A higher value means a greater difference. 
...
```{r}
sig_interRel <- threshSigInter(inter0, outs_background, pvalue = 1)
```

This function uses the background distribution to set a pvalue threshold, which is then applied to the value column. Usually, the pvalue is set to 0.05, but I set it to 1 here to capture all possible relationships, regardless of significance. 
...
In the example data, V1 is the miRNA id, V2 is the gene id, V3 is the score, V4 is energy in kcal/mol, V5 is the subject % identify, and V6 is the query % identity. In my miranda data, V1 is the miRNA id, V2 is the gene id, V3 is the score, V4 is energy in kcal/mol, V5 is start and end miRNA, V6 is start and end mRNA, V7 is the total number of bp overlap, V8 is the subject % identify, and V9 is the query % identity.

Select these columns from my data, rearrange columns, and rename columns to V1-6. This will allow the following functions to properly read the miranda file. 
```{r}
# Select specific columns
miranda_data_filt <- miranda_data %>%
  dplyr::select(V1, V2, V3, V4, V8, V9)

# Remove any duplicate rows
miranda_data_filt <- unique(miranda_data_filt)

# Rename columns
colnames(miranda_data_filt) <- c("V1", "V2", "V3", "V4", "V5", "V6")
```

Edit miRNA and mRNA names to remove extra information. 
```{r}
# mirna names 
miranda_data_filt$V1 <- miranda_data_filt$V1 %>%
  str_remove("^>") %>%                  # Remove ">" at the beginning
  str_replace("\\.mature::", "_") %>%   # Replace ".mature::" with "_"
  str_remove(":.*$")                     # Remove everything after and including ":"

# mrna names 
miranda_data_filt$V2 <- miranda_data_filt$V2 %>%
  str_remove("ID=") %>%          # Remove "ID="
  str_remove(";.*$")             # Remove everything after and including the first ";"
```
...
Here, the putative interactions as predicted by miranda are used to filter down the initial correlations. We are left with 1 miRNA that binds with 2 different genes, but these interactions were not significant, as calculated by mirtarrnaseq. 

I can't plot using `mirRnaHeatmapDiff`, as there must be greater than 2 objects to properly cluster. 

Combine results data into a single df and write as csv 
```{r}
...
# Better plot
plot1 <- ggplot(combined_df_subset, aes(x = mirna_FC, y = mrna_FC)) +
...
colnames(TP0_v_7_amb_df) <- c("gene_id", "log2FoldChange", "pvalue")
```
...
Read in miRNA file of interest
```{r}
TP0_v_7_amb_miRNA_df <- read.csv("../output/Molecular/smRNA/shortstack/TP0_v_7_amb_miRNA.csv") %>%
  dplyr::select(X, log2FoldChange, padj)# %>%
  #rename("gene_id" = "X", "pvalue" = "padj")

colnames(TP0_v_7_amb_miRNA_df) <- c("gene_id", "log2FoldChange", "pvalue")
```
...
Miranda data already read in above.

Intersect miranda data with significant relationships
```{r}
results_TP0_v_TP7_amb <- mirandaIntersectInter(sig_interRel, outs_background, mrna_data, mirna_data, miranda_data_filt)
tail(results_TP0_v_TP7_amb$corrs)
```

Still no significant pvalues. Maybe if I change the background distribution?
...
Make heatmap for  relationships
```{r}
mirRnaHeatmapDiff(results_TP0_v_TP7_amb$corrs, fontsize = 10)

pdf("../output/Molecular/interactions/TP0_v_TP7_amb_interactions.pdf")
...
Plot FCs
```{r}
plot2 <- ggplot(combined_df, aes(x = mirna_FC, y = mrna_FC)) +
  geom_point(size = 4, shape = 21, color = "black", fill = "black") +  # Customize points
  #geom_vline(xintercept = 0, linetype = "solid", color = "black", size = 0.5) +  # Add vertical line at x = 0
  #geom_hline(yintercept = 0, linetype = "solid", color = "black", size = 0.5) +  # Add horizontal line at y = 0
  #geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed", size = 1) +  # Customize regression line
...
Read in miRNA file of interest
```{r}
TP0_v_5_heat_miRNA_df <- read.csv("../output/Molecular/smRNA/shortstack/TP0_v_5_heat_miRNA.csv") %>%
  dplyr::select(X, log2FoldChange, padj) %>%
  dplyr::rename(gene_id = X, pvalue = padj)
```
...
tail(results_TP0_v_TP5_heat$corrs)
```
...
Make heatmap for  relationships
```{r}
mirRnaHeatmapDiff(results_TP0_v_TP5_heat$corrs, fontsize = 10)
```
...
Plot FCs
```{r}
plot3 <- ggplot(combined_df, aes(x = mirna_FC, y = mrna_FC)) +
  geom_point(size = 4, shape = 21, color = "black", fill = "black") +  # Customize points
  #geom_vline(xintercept = 0, linetype = "solid", color = "black", size = 0.5) +  # Add vertical line at x = 0
  #geom_hline(yintercept = 0, linetype = "solid", color = "black", size = 0.5) +  # Add horizontal line at y = 0
  #geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed", size = 1) +  # Customize regression line
...
ggsave("../output/Molecular/interactions/TP0_amb_v_TP5_heat_FC_plot.pdf", plot3, width = 10, height = 8)
ggsave("../output/Molecular/interactions/TP0_amb_v_TP5_heat_FC_plot.png", plot3, width = 10, height = 8)
```
...
OLD CODE as of 11/18/24

Join miranda data with inter0 and FC data 
```{r}
# mrna FC data 
mrna_data$gene <- row.names(mrna_data)

inter0_fc <- inter0 %>%
  full_join(mrna_data, by = c("V2" = "gene"))

# mirna FC data 
mirna_data$mirna <- row.names(mirna_data)

inter0_fc <- inter0_fc %>%
  full_join(mirna_data, by = c("V1" = "mirna"))

# Rename 
colnames(inter0_fc) <- c("mirna", "gene", "value", "mRNA_FC", "miRNA_FC")
```

Plot
```{r}
ggplot(inter0_fc, aes(x = miRNA_FC, y = mRNA_FC)) +
...
the two miRNAs are essentially regulating the same things...
