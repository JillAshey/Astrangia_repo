...
my.data_amb <- test %>% 
  select(Date.Time, Temp_Amb, Temp_Amb_StdDev, upper, lower) %>%
  rename("Temperature" = "Temp_Amb") %>%
  rename("sd" = "Temp_Amb_StdDev") %>%
...
# Select specific columns 
my.data_heat <- test2 %>% 
  select(Date.Time, Temp_Heat, Temp_Heat_StdDev, upper, lower) %>%
  rename("Temperature" = "Temp_Heat") %>%
  rename("sd" = "Temp_Heat_StdDev") %>%
  #separate(col = Date.Time, into = c("Date", "Time"), sep = " ") %>%
  na.omit()
...
```{r}
noaa_data <- noaa_data %>%
  select(Date.Time, Temperature, sd, upper, lower, Treatment)
```
...
ggsave("../output/Environmental/MyData_NOAA_temperature_smooth.png", temp_plot, width=6, height=4)
ggsave("../output/Environmental/MyData_NOAA_temperature_smooth.pdf", temp_plot, width=6, height=4)
```
