...
# Assess number of miRNA and mRNA in interactions
dim(miranda_data)
length(unique(miranda_data$V1))
length(unique(miranda_data$V2))
```
...
```{r}
combined_data_pcc_deg_annot <- combined_data_pcc_deg %>%
  left_join(annot, by = c("mRNA" = "Protein_ID")) %>%
...
Merge with annotation data 
```{r}
combined_data_pcc_sig_deg_dem_annot <- combined_data_pcc_sig_deg_dem %>%
  left_join(annot, by = c("mRNA" = "Protein_ID")) %>%
  select(-starts_with("AST"))
```
