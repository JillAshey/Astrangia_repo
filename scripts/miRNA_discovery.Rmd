...
Rename miRNAs so that it includes just the cluster name 
```{r}
counts$miRNA_name <- sub("^(([^_]+_){1}[^_]+).*", "\\1", counts$miRNA_name)
```
...
Save putative miRNAs
```{r}
write.csv(counts, "../output/Molecular/smRNA/shortstack/putative_miRNA_counts.csv")
```
...
# Join with metadata
counts_meta <- left_join(counts_long, meta, by = "ID")

miRNA_means <- counts_meta %>%
  group_by(miRNA, Timepoint) %>%
  summarise(mean_count = mean(counts, na.rm = TRUE), .groups = "drop")
...
ggplot(miRNA_means, aes(x = Timepoint, y = mean_count, group = miRNA, color = miRNA)) +
  ...
  labs(title = "miRNA Expression Trajectories",
       ...
       color = "miRNA Cluster") +
  theme_minimal() +
  theme(legend.position = "none")  # Hide legend if too many miRNAs
```
