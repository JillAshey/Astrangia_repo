...
# renaming P to Photosynthesis and Rd to Respiration 
sub$Assay[sub$Assay == "P"] <- "Photosynthesis"
sub$Assay[sub$Assay == "Rd"] <- "Respiration"
...
ggsave("output/Physiology/PR/PR_Treatment_20230424.pdf", pr_plot, width = 50, height = 25, units = "cm")
ggsave("output/Physiology/PR/PR_Treatment_20230424.png", pr_plot, width = 50, height = 25, units = "cm")
```
...
p_df <- sub %>%
  filter(Assay == "Photosynthesis")
rd_df <- sub %>%
  filter(Assay == "Respiration")

## Plot P
p_plot <- ggplot(p_df, aes(x = Timepoint, y = micromol.cm2.h, fill = Treatment)) +
  geom_boxplot(lwd = 1, color = "black") +
  scale_fill_manual(values = c("blue", "red")) +
  #ylab("Carbohydrates (mg/cm²)") +
  xlab("") +
  ...
  theme(axis.text.x = element_text(size = 30,
   ...
   theme(axis.text.y = element_text(size = 30,
        ...
        axis.title.y = element_text(size = 35,
                                    margin = margin(t = 0, r = 20, b = 0, l = 0))) 
p_plot
ggsave("output/Physiology/PR/P_Treatment_20230424.pdf", p_plot, width = 50, height = 25, units = "cm")
ggsave("output/Physiology/PR/P_Treatment_20230424.png", p_plot, width = 50, height = 25, units = "cm")

## Plot R
r_plot <- ggplot(rd_df, aes(x = Timepoint, y = micromol.cm2.h, fill = Treatment)) +
  geom_boxplot(lwd = 1, color = "black") +
  scale_fill_manual(values = c("blue", "red")) +
  #ylab("Carbohydrates (mg/cm²)") +
  xlab("") +
  ...
  theme(axis.text.x = element_text(size = 30,
   ...
   theme(axis.text.y = element_text(size = 30,
        ...
        axis.title.y = element_text(size = 35,
                                    margin = margin(t = 0, r = 20, b = 0, l = 0))) 
r_plot
ggsave("output/Physiology/PR/R_Treatment_20230424.pdf", r_plot, width = 50, height = 25, units = "cm")
ggsave("output/Physiology/PR/R_Treatment_20230424.png", r_plot, width = 50, height = 25, units = "cm")

################################# PHOTOSYNTHESIS ################################# 
## Compute two-way ANOVA test 
model <- aov(log(micromol.cm2.h) ~ Treatment*Timepoint, data = p_df)
summary(model)
...
## TP1 differs from TP5, TP6, & TP7

# Multiple comparisons using multcomp package

# Pairwise test 
pairwise.t.test(p_df$micromol.cm2.h, p_df$Timepoint) # should I specify a p.adjust.method? 
...
all <- all %>% 
  mutate(Month = case_when(
    Date == "2021-02-04" ~ "Febuary", # TP0
    Date == "2021-03-04" ~ "March", # TP1
    Date == "2021-04-01" ~ "Early April", # TP2
    Date == "2021-04-10" ~ "Early April", # TP2
    Date == "2021-04-29" ~ "Late April", # TP3
    Date == "2021-04-30" ~ "Late April", # TP3
    Date == "2021-05-27" ~ "May", # TP4
    Date == "2021-06-02" ~ "May", # TP4
    Date == "2021-06-24" ~ "June", # TP5
    Date == "2021-06-25" ~ "June", # TP5
    Date == "2021-07-22" ~ "July", # TP6
    Date == "2021-07-23" ~ "July", # TP6
    Date == "2021-08-19" ~ "August", # TP7
    Date == "2021-09-01" ~ "August")) # TP7
...
Remove P assay because most colonies were aposymbiotic 
```{r}
all_rd <- all %>%
  filter(Assay == "Rd")
```

Remove any values that were greater than 0, as positive values should not have occurred during the dark respiration run, as well as the acclimatization period (TP0) and field data.
```{r}
all_rd <- all_rd %>%
  #filter(!micromol.cm2.h > 0) %>%
  filter(!micromol.cm2.h < -10) %>%
  #filter(!Treatment == "Acclimatization") %>%
  filter(!Treatment == "Field")
```

Set levels for plotting by month 
```{r}
all_rd$Month <- factor(all_rd$Month, levels = c("Febuary", "March", "Early April", "Late April", "May", "June", "July", "August"))
```

Plot
```{r}
plot <- ggplot(all_rd, aes(x = Month, y = micromol.cm2.h, colour = Treatment)) +
  ...
  #geom_hline(yintercept = 0, linetype = "dotted", color = "black", size = 1) +  # Add dotted line at y = 0
...
ggsave("../output/Physiology/PR/R_Treatment_20240521.pdf", plot, width = 15, height = 10)
ggsave("../output/Physiology/PR/R_Treatment_20240521.png", plot, width = 15, height = 10)
```

Summarize data
```{r}
## Summarize data w/o field 
mean.r_exp <- summarySE(all_rd, measurevar = "micromol.cm2.h", groupvars = c("Treatment", "Timepoint"))
#mean.r_site_exp <- summarySE(carb, measurevar = "micromol.cm2.h", groupvars = c("Site", "Treatment", "Timepoint"))
```

Going to do stats just for Temperate coral conference 
```{r}
model <- aov(micromol.cm2.h ~ Treatment*Timepoint, data = all_rd)
summary(model)

#                      Df Sum Sq Mean Sq F value   Pr(>F)    
# Treatment             2  10.87   5.436   3.027   0.0524 .  
# Timepoint             6  87.07  14.512   8.081 2.92e-07 ***
# Treatment:Timepoint   6  60.81  10.135   5.644 3.76e-05 ***
# Residuals           112 201.13   1.796                     
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
# b. Levene's test 
leveneTest(residuals(model) ~ Timepoint*Treatment, data = all_rd)
## p-value is less than 0.05, so variance is not homogeneous 
...
# Tukey HSD 
TukeyHSD(model, which = "Timepoint")
## TP1 differs from TP6; TP2, TP3, TP5, TP6 and TP7 differs from TP4

TukeyHSD(model, which = "Treatment")
## Ambient and acclimation differed
```

Probably need to do data transformations 

It is weird that there are positive values in the R data since the corals were in the dark. 
...
Set levels for plotting by month 
```{r}
all_p$Month <- factor(all_p$Month, levels = c("Febuary", "March", "Early April", "Late April", "May", "June", "July", "August"))
```

Plot
```{r}
plot <- ggplot(all_p, aes(x = Month, y = micromol.cm2.h, colour = Treatment)) +
  geom_boxplot(aes(colour=Treatment), lwd=2) +
  #geom_point(aes(fill=Treatment, group=Treatment), colour="black", pch = 21, size=4, position = position_jitterdodge(0.3), alpha=0.9) + 
  #geom_hline(yintercept = 0, linetype = "dotted", color = "black", size = 1) +  # Add dotted line at y = 0
  scale_fill_manual(values = c("grey", "blue", "red")) +
...
ggsave("../output/Physiology/PR/P_Treatment_20240521.pdf", plot, width = 15, height = 10)
ggsave("../output/Physiology/PR/P_Treatment_20240521.png", plot, width = 15, height = 10)
```

Summarize data
```{r}
## Summarize data w/o field 
mean.p_exp <- summarySE(all_p, measurevar = "micromol.cm2.h", groupvars = c("Treatment", "Timepoint"))
#mean.r_site_exp <- summarySE(carb, measurevar = "micromol.cm2.h", groupvars = c("Site", "Treatment", "Timepoint"))
```

Going to do stats just for Temperate coral conference 
```{r}
model <- aov(micromol.cm2.h ~ Treatment*Timepoint, data = all_p)
summary(model)

#                      Df Sum Sq Mean Sq F value   Pr(>F)    
# Treatment             2     11    5.29   0.166 0.846917    
# Timepoint             6    906  151.02   4.747 0.000239 ***
# Treatment:Timepoint   6    137   22.89   0.719 0.634742    
# Residuals           113   3595   31.82                     
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
# b. Levene's test 
leveneTest(residuals(model) ~ Timepoint*Treatment, data = all_p)
...
# Tukey HSD 
TukeyHSD(model, which = "Timepoint")
## TP2 differs from TP4 and TP6; TP3 differs from TP4
```
