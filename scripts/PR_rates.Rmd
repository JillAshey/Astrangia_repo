...
```{r}
all_rd$Month <- factor(all_rd$Month, levels = c("March", "Early April", "Late April", "May", "June", "July", "August"))
```
...
## Likely have to remove May, as there are only 2 points for Heat trt 

ggsave("../output/Physiology/PR/R_Treatment_20240108.pdf", plot, width = 10, height = 8)
ggsave("../output/Physiology/PR/R_Treatment_20240108.png", plot, width = 10, height = 8)
```
...
Stats!
```{r}
## Need to remove May, as there are only two points there after filtering 
all_rd <- all_rd %>% 
  filter(!Month == "May")

## Change all values to positive (aka inverse)
all_rd <- all_rd %>%
  mutate(micromol.cm2.h_inverse = micromol.cm2.h*(-1))

## Compute two-way ANOVA test 
model <- aov(micromol.cm2.h_inverse ~ Treatment*Timepoint, data = all_rd)
summary(model)

## Slight significance by time point 
...
## Compute two-way ANOVA test 
model <- aov(log(micromol.cm2.h_inverse) ~ Treatment*Timepoint, data = all_rd)
summary(model)
...
# b. Levene's test -- cant run with log, produces an error
leveneTest(residuals(model) ~ Timepoint*Treatment, data = all_rd)
## p-value is  less than 0.05, so variance is not homogenous 
...
aov_residuals <- residuals(object = model)
shapiro.test(x = aov_residuals)
# pvalue is greater than 0.05, normality attained
...
## Compute two-way ANOVA test 
model <- aov(sqrt(micromol.cm2.h_inverse) ~ Treatment*Timepoint, data = all_rd)
summary(model)
...
# b. Levene's test -- cant run with sqrt, produces an error
leveneTest(residuals(model) ~ Timepoint*Treatment, data = all_rd)
## p-value is  less than 0.05, so variance is not homogenous 
...
## Compute two-way ANOVA test 
model <- aov((micromol.cm2.h_inverse)^2 ~ Treatment*Timepoint, data = all_rd)
summary(model)
...
# b. Levene's test -- cant run with sqrt, produces an error
#leveneTest(sqrt(micromol.cm2.h) ~ Timepoint*Treatment, data = all_rd)
## p-value is  less than 0.05, so variance is not homogenous 
...
aov_residuals <- residuals(object = model)
shapiro.test(x = aov_residuals)
# pvalue is less than 0.05, normality violated
```

Log and sqrt both seem like good potential transformations for this data
...
Quants <- all_rd %>%
  ...
  summarize(quant25 = quantile(all_rd, probs = q[1]),
            quant75 = quantile(all_rd, probs = q[2]),
            IQRbyGroup=IQR(all_rd))
```