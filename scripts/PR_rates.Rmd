...
pr.save <- pr.save %>% 
  mutate(Timepoint = case_when(
    ...
    Date == "2021-05-27" ~ "Time4",
    Date == "2021-06-24" ~ "Time5"))
...
## Import data
```{r}
path.p <- "data/PR/raw_field" #the location of all your respirometry files 
...
# Load PI curve sample metadata (i.e., which corals were in which runs)
sample.info <- read_csv(file = "data/PR/PR_field_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/PR/PR_field_run_metadata.csv")
...
# Write to output file
write.csv(pr.out, "output/PR_field_rates.csv")
```
...
pr.save <- pr.save %>% 
  mutate(Timepoint = case_when(
    Date == "2021-04-10" ~ "FLD-Time2",
    Date == "2021-04-30" ~ "FLD-Time3",
    Date == "2021-06-02" ~ "FLD-Time4",
    Date == "2021-06-25" ~ "FLD-Time5"))
...
ggsave("output/PR/Field_Timepoint.Treatment_PR.pdf", plot, width = 4, height = 3.5, units = c("in"))
...
ggsave("output/PR/Combination.Exp.Fld_Timepoint.Treatment_PR.pdf", plot, width = 4, height = 7, units = c("in"))
...
### Import data 
```{r}
exp <- read_csv(file = "output/PR_rates.csv")
field <- read_csv(file = "output/PR_field_rates.csv")
...
all <- all %>% 
  mutate(Timepoint = case_when(
    ...
    Date == "2021-03-04" ~ "Time1",
    Date == "2021-04-01" ~ "Time2",
    Date == "2021-04-10" ~ "Time2",
    Date == "2021-04-29" ~ "Time3",
    Date == "2021-04-30" ~ "Time3",
    Date == "2021-05-27" ~ "Time4",
    Date == "2021-06-02" ~ "Time4",
    Date == "2021-06-24" ~ "Time5",
    Date == "2021-06-25" ~ "Time5")) 
...
ggsave("output/PR/All_Timepoint.Treatment_PR.pdf", plot, width = 4, height = 3.5, units = c("in"))
```
