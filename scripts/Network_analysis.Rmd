...
# STEP 4: Create GO enrichment plots for each cluster
# ==========================================
# Get top N GO terms per cluster from your new June treatment results
top_n_terms <- 10  
...
go_data_plot <- june_trt_functions %>%
  ...
  filter(ontology == "Biological Processes") %>%
  group_by(cluster) %>%
  # Ensure Fisher is numeric for sorting
...
go_plots <- lapply(all_clusters, function(clust) {
  ...
  ggplot(go_data, aes(x = neglog10p, y = Term_ordered, size = Significant)) + 
