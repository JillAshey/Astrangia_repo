...
timepoint_summary <- seasonal_all_correlations_high_miRNA %>%
  group_by(timepoint) %>%
  summarise(
    unique_miRNAs = n_distinct(miRNA),
    unique_mRNAs = n_distinct(mRNA),
    unique_pairs = n_distinct(pair_id))
```
...
features$raw_cluster <- as.factor(clusters$cluster)

# Join with pair trajectories df 
pair_trajectories <- pair_trajectories %>%
  left_join(features %>% select(pair_id, raw_cluster), by = "pair_id") %>%
  # Explicitly remap k-means clusters to match the desired 1:5 trajectory shapes
  mutate(
    cluster = case_match(
      raw_cluster,
      "4" ~ "1",  # Peak in June
    ...
    cluster = factor(cluster, levels = c("1", "2", "3", "4", "5"))
  )

cluster_summary <- pair_trajectories %>%
  group_by(cluster) %>%
...
```{r}
write.csv(pair_trajectories, "../output/Molecular/interactions/networks/seasonal_cluster_trajectory.csv")
```
...
trajectory_long <- pair_trajectories %>%
  ...
  mutate(season = case_match(timepoint, "TP0" ~ "Feb", "TP5" ~ "June", "TP7" ~ "Aug", .default = timepoint))
...
subset_cluster1 <- pair_trajectories %>%
  dplyr::filter(cluster == "1", TP0 < 0, TP5 > 0, TP7 < 0)

length(unique(subset_cluster1$miRNA))
length(unique(subset_cluster1$mRNA))
length(unique(subset_cluster1$pair_id))
```
...
length(unique(subset_cluster2$miRNA))
length(unique(subset_cluster2$mRNA))
length(unique(subset_cluster2$pair_id))
```
...
length(unique(subset_cluster3$miRNA))
length(unique(subset_cluster3$mRNA))
length(unique(subset_cluster3$pair_id))
```
...
length(unique(subset_cluster4$miRNA))
length(unique(subset_cluster4$mRNA))
length(unique(subset_cluster4$pair_id))
```
...
length(unique(subset_cluster5$miRNA))
length(unique(subset_cluster5$mRNA))
length(unique(subset_cluster5$pair_id))
```
...
# Save as csv 
write.csv(seasonal_functions, "../output/Molecular/interactions/networks/seasonal_clusters_functional.csv")
```

Read in seasonal functional data for plotting 
```{r}
seasonal_functions <- read.csv("../output/Molecular/interactions/networks/seasonal_clusters_functional.csv")
```
...
trajectory_long <- pair_trajectories %>%
  left_join(features %>% select(pair_id), by = "pair_id") %>%
  pivot_longer(cols = starts_with("TP"), names_to = "timepoint", values_to = "cor") %>%
  mutate(season = dplyr::recode(timepoint, "TP0" = "February", "TP5" = "June", "TP7" = "August")) %>%
...
# ==========================================
# STEP 2: Prepare GO enrichment data (Fold Enrichment)
# ==========================================
...
go_data_plot <- seasonal_functions %>%
  ...
  mutate(
    ...
    # Prevent division by zero if Expected is 0
    Expected_safe = ifelse(Expected == 0, 0.01, Expected),
    # Calculate Fold Enrichment
    Enrichment_raw = Significant / Expected_safe,
    # Cap extreme single-gene outliers at 50 for balanced dot sizing
    Enrichment    = pmin(Enrichment_raw, 50)
  ) %>%
  group_by(cluster) %>%
  arrange(Fisher) %>%
  ...
  mutate(
    ...
    # Unique factor per cluster to keep terms ordered by significance
    term_clust   = paste(Term_short, cluster, sep = "___"),
    Term_ordered = reorder(term_clust, neglog10p)
  )

# Set global scale limits so sizes are directly comparable across all clusters
enrich_limits <- range(go_data_plot$Enrichment[is.finite(go_data_plot$Enrichment)], na.rm = TRUE)
...
go_plots <- lapply(unique(go_data_plot$cluster), function(clust) {
  ...
  if (nrow(go_data) == 0) {
    return(ggplot() + annotate("text", x = 0.5, y = 0.5, label = "No enrichment") + theme_void())
  }
  
  ggplot(go_data, aes(x = neglog10p, 
                      y = Term_ordered,
                      size = Enrichment)) + 
    geom_point(alpha = 0.8, color = "black") + 
    # Clean up y-axis labels
    scale_y_discrete(labels = function(x) gsub("___.*$", "", x)) +
    # Unified size scale with pretty breaks
    ...
    labs(
      ...
      title = paste("Cluster", clust, "Functions")
    ) +
    theme_bw(base_size = 11) +
    theme(
      plot.title         = element_text(face = "bold", size = 12, hjust = 0.5),
...
names(go_plots) <- unique(go_data_plot$cluster)
...
# 1. Get unique cluster IDs to ensure alignment
all_clusters <- as.character(unique(cluster_stats$cluster))

# 2. Create Trajectory Plots (Named List)
trajectory_plots <- lapply(all_clusters, function(clust) {
  traj_data <- trajectory_long %>% filter(cluster == clust)
  stat_data <- cluster_stats %>% filter(cluster == clust)
...
names(trajectory_plots) <- all_clusters

# 3. Create GO enrichment plots (Named List)
# We iterate over the same 'all_clusters' to maintain order/placeholders
all_cluster_ids <- unique(cluster_stats$cluster)

go_plots <- lapply(all_cluster_ids, function(clust) {
  
  # Filter data for this cluster
  go_data <- go_data_plot %>% filter(cluster == clust)
  
  # CHECK: If there are no GO terms, return a placeholder to avoid "out of bounds"
  if (nrow(go_data) == 0) {
    return(
      ggplot() + 
  ...
  # If data exists, create the dot plot
...
# Name the lists so they can be accessed by Cluster ID string
names(go_plots) <- all_cluster_ids
names(trajectory_plots) <- all_cluster_ids

# 4. Combine trajectory + GO for each cluster into a list of "rows"
cluster_combined <- lapply(all_clusters, function(clust) {
  trajectory_plots[[clust]] + go_plots[[clust]] + plot_layout(widths = c(1, 1.2))
})

# 5. Final Assembly
# tag_levels = 'A' automatically adds A, B, C... to the subplots
cluster_combined <- lapply(all_cluster_ids, function(clust) {
  trajectory_plots[[as.character(clust)]] + go_plots[[as.character(clust)]] + 
    plot_layout(widths = c(1, 1.2))
})

# Wrap everything into one vertical column
# tag_levels = 'A' will label the trajectory/GO pairs as A, B, C, D, E...
final_plot <- wrap_plots(cluster_combined, ncol = 1) + 
...
## Treatment network metrics 

Read in network metrics data again if needed
```{r}
network_metrics <- read.csv("../output/Molecular/interactions/networks/network_metrics.csv")
```

Subset network metrics by treatment
```{r}
treatment_network_metrics <- network_metrics %>%
  filter(condition %in% c("TP5_Ambient", "TP5_Heat", "TP7_Ambient", "TP7_Heat"))
```

Make PCA of treatment network metrics 
```{r}
# Make PCA 
metrics <- treatment_network_metrics[, c("degree", "betweenness", "dprime")]
pca <- prcomp(metrics, center = TRUE, scale. = TRUE)

# Extract scores 
scores <- as.data.frame(pca$x)
scores$condition <- treatment_network_metrics$condition
scores$miRNA <- treatment_network_metrics$miRNA
...
Make violin plots for treatment network metrics 
```{r}
# Ensure condition is ordered chronologically
treatment_network_metrics <- treatment_network_metrics %>%
  mutate(condition = factor(condition, 
                           levels = c("TP5_Ambient", "TP5_Heat", "TP7_Ambient", "TP7_Heat")))

# Degree plot 
degree_treatment_violin <- ggplot(treatment_network_metrics, aes(x = condition, y = degree, fill = condition)) +
  geom_violin(alpha = 0.7, trim = FALSE) +
  geom_boxplot(width = 0.15, fill = "white", outlier.alpha = 0) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "none",
  ...
  labs(x = "Condition", 
       y = "Degree (number of mRNA targets)",
       title = "miRNA degree distribution across treatment timepoints"); degree_treatment_violin

# Betweenness plot 
betweenness_treatment_violin <- ggplot(treatment_network_metrics, aes(x = condition, y = betweenness, fill = condition)) +
  geom_violin(alpha = 0.7, trim = FALSE) +
  geom_boxplot(width = 0.15, fill = "white", outlier.alpha = 0) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "none",
  ...
  labs(x = "Condition", 
       y = "Betweenness",
       title = "miRNA betweenness distribution across treatment timepoints"); betweenness_treatment_violin

# dprime plot 
dprime_treatment_violin <- ggplot(treatment_network_metrics, aes(x = condition, y = dprime, fill = condition)) +
  geom_violin(alpha = 0.7, trim = FALSE) +
  geom_boxplot(width = 0.15, fill = "white", outlier.alpha = 0) +
  theme_linedraw(base_size = 12) +
  theme(legend.position = "none",
  ...
  labs(x = "Condition", 
       y = "d'",
       title = "miRNA d' distribution across treatment timepoints"); dprime_treatment_violin
```

Test for differences in degree, betweenness and dprime by season
```{r}
# KRUSKAL-WALLIS TESTS (non-parametric)
# Define the specific comparisons we want to make
comparisons <- list(
  TP5 = c("TP5_Ambient", "TP5_Heat"),
  TP7 = c("TP7_Ambient", "TP7_Heat")
)

# Function to run Wilcoxon for each metric within a timepoint
run_comparisons <- function(df, timepoint_levels) {
  df_filtered <- df %>% filter(condition %in% timepoint_levels)
  
  data.frame(
    degree_p = wilcox.test(degree ~ condition, data = df_filtered)$p.value,
    betweenness_p = wilcox.test(betweenness ~ condition, data = df_filtered)$p.value,
    dprime_p = wilcox.test(dprime ~ condition, data = df_filtered)$p.value
  )
}

# Apply the function to each comparison
results <- map_df(comparisons, ~run_comparisons(treatment_network_metrics, .x), .id = "timepoint")

print(results)
```

## Treatment correlation trajectories 

Read in correlation data from above 
```{r}
...
Filter correlation data for June ambient and heat samples 
```{r}
june_trt_all_correlations_high_miRNA <- all_correlations_high_miRNA %>%
  filter(timepoint %in% c("TP5")) %>%
  filter(treatment %in% c("Ambient", "Heat"))

june_summary <- june_trt_all_correlations_high_miRNA %>%
...
Reshape df for trajectory calculations
```{r}
june_trajectories <- june_trt_all_correlations_high_miRNA %>%
  dplyr::select(pair_id, miRNA, mRNA, timepoint, treatment, cor_value) %>%
  dplyr::group_by(pair_id, miRNA, mRNA, timepoint, treatment) %>%
  dplyr::summarise(cor_value = mean(cor_value, na.rm = TRUE), .groups = "drop") %>%
  # Drop time_num here, keep only what's needed for pivot
  #select(-time_num) %>%
  pivot_wider(names_from = treatment, values_from = cor_value, 
              values_fill = NA, values_fn = mean) %>%
  # Add back complete trajectory filter
  filter(!is.na(Ambient), !is.na(Heat))
```

Fit trajectories
```{r}
fit_pair_trajectory <- function(row) {
  # We only have two points, so we use a linear model (degree 1)
  df <- data.frame(treatment = c(1, 2), cor = c(row$Ambient, row$Heat))
  mod <- lm(cor ~ treatment, data = df) # Simple linear model
  ...
  broom::tidy(mod) %>%
    ...
    dplyr::select(pair_id, miRNA, mRNA, term, estimate) %>%
    pivot_wider(names_from = term, values_from = estimate)
}

june_pair_fits <- june_trajectories %>%
  rowwise() %>%
  do(fit_pair_trajectory(.)) %>%
  ungroup()
```
...
Cluster trajectories 
```{r}
# Calculate the 'trajectory' features for two points
features <- june_trajectories %>%
  mutate(
    slope = Heat - Ambient,      # Direction and magnitude of change
    intercept = Ambient          # Starting point (Baseline)
  ) %>%
  dplyr::select(pair_id, slope, intercept)

# Standardize to ensure slope and intercept are weighted equally
features_scaled <- scale(features[, c("slope", "intercept")])

# Run K-means with the chosen k
set.seed(123)
k_opt <- 3 
clusters <- kmeans(features_scaled, centers = k_opt, nstart = 100)
features$cluster <- as.factor(clusters$cluster)

# Join with pair trajectories df 
june_trajectories <- june_trajectories %>%
  left_join(features %>% dplyr::select(pair_id, cluster), by = "pair_id")

cluster_summary <- june_trajectories %>%
  group_by(cluster) %>%
  summarise(unique_pairs = n_distinct(pair_id),
            unique_miRNAs = n_distinct(miRNA),
            unique_mRNAs = n_distinct(mRNA))
...
Save trajectory info
```{r}
write.csv(june_trajectories, "../output/Molecular/interactions/networks/june_trt_cluster_trajectory.csv")
```

Plot trajectories 
```{r}
# Prepare long format data (same as before)
june_treatment_long <- june_trajectories %>%
  left_join(features %>% dplyr::select(pair_id, cluster), by = c("pair_id", "cluster")) %>%
  # Pivot Ambient and Heat into one column
  pivot_longer(cols = c(Ambient, Heat), 
               names_to = "treatment", 
               values_to = "cor") %>%
  # Ensure Ambient comes first on the x-axis
  mutate(treatment = factor(treatment, levels = c("Ambient", "Heat")))

# Calculate Stats (Mean ± SE) per cluster and treatment
cluster_stats <- june_treatment_long %>%
...
p_faceted <- ggplot() +
  # Faint individual reaction norms (slopes) for each pair
  geom_line(data = june_treatment_long, 
            aes(x = treatment, y = cor, group = pair_id), 
            alpha = 0.08, color = "grey60", linewidth = 0.4) +
  
  # Mean trajectory per cluster (The "Average" response)
  geom_line(data = cluster_stats, 
            aes(x = treatment, y = mean_cor, group = cluster), 
            color = "red3", linewidth = 1.3) +
  
  # Error bars or ribbons for the mean
  geom_errorbar(data = cluster_stats,
                aes(x = treatment, ymin = mean_cor - se_cor, ymax = mean_cor + se_cor),
                width = 0.1, color = "red3", linewidth = 1) +
  
  geom_point(data = cluster_stats, 
             aes(x = treatment, y = mean_cor), 
             color = "red3", size = 2.8) +
  ...
  labs(x = "Treatment", 
       y = "Correlation (miRNA-mRNA)",
       title = "Thermal Response of miRNA-mRNA Pairs by Cluster",
       subtitle = "June Timepoint: Ambient vs. Heat Stress") +
  theme_bw(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold", size = 11),
    panel.grid.minor = element_blank(),
...
## Enrichment of June treatment clusters 
...
### Cluster 1 - June treatment 

Subset by Cluster 1 and select only negative correlations for Ambient and positive correlations for Heat 
```{r}
subset_cluster1 <- june_trajectories %>%
  dplyr::filter(cluster == "1", Ambient < 0, Heat > 0)
...
#### Biological Processes - cluster 1 June treatment
...
#### Molecular Functions - cluster 1 June treatment
...
```{r}
cluster1_june_trt_functional <- rbind(GO_BP_En_sig, GO_MF_En_sig)

# Save as csv 
write.csv(cluster1_june_trt_functional, "../output/Molecular/interactions/networks/june_trt_cluster1_functional.csv")
```

### Cluster 2 - June treatment 

Subset by Cluster 2 and select only negative correlations for Heat and positive correlations for Ambient 
```{r}
subset_cluster2 <- june_trajectories %>%
  dplyr::filter(cluster == "2", Heat < 0, Ambient > 0)
...
#### Biological Processes - cluster 2 June treatment
...
Generate results table 
```{r}
GO_BP_En <- GenTable(GO_BP, Fisher = GO_BP_FE, orderBy = "Fisher", numChar = 51, topNodes = 175)
```
...
#### Molecular Functions - cluster 2 June treatment
...
```{r}
cluster2_june_trt_functional <- rbind(GO_BP_En_sig, GO_MF_En_sig)

# Save as csv 
write.csv(cluster2_june_trt_functional, "../output/Molecular/interactions/networks/june_trt_cluster2_functional.csv")
```

### Cluster 3 - June treatment 

Subset by Cluster 3 and select only negative correlations for both Ambient and Heat
```{r}
subset_cluster3 <- june_trajectories %>%
  dplyr::filter(cluster == "3", Heat < 0, Ambient < 0)
...
#### Biological Processes - cluster 3 June treatment
...
#### Molecular Functions - cluster 3 June treatment
...
```{r}
cluster3_june_trt_functional <- rbind(GO_BP_En_sig, GO_MF_En_sig)

# Save as csv 
write.csv(cluster3_june_trt_functional, "../output/Molecular/interactions/networks/june_trt_cluster3_functional.csv")
```

## Plot June treatment functional results 

Bind functional results from all clusters 
```{r}
june_trt_functions <- rbind(cluster1_june_trt_functional, cluster2_june_trt_functional, cluster3_june_trt_functional)

# Save as csv 
write.csv(june_trt_functions, "../output/Molecular/interactions/networks/june_trt_clusters_functional.csv")
```

Read in functional info if needed for plotting 
```{r}
june_trt_functions <- read.csv("../output/Molecular/interactions/networks/june_trt_clusters_functional.csv")
```

Read in DEGs
```{r}
june_degs <- read_csv("../output/Molecular/mRNA/TP5_amb_v_TP5_heat_DEG.csv")
june_degs <- june_degs %>%
  rename("mRNA" = "...1") 
```

Check if any DEGs are in the clusters
```{r}
june_deg_list <- june_degs$mRNA
june_trj_list <- june_trajectories$mRNA
common_terms <- Reduce(intersect, list(june_deg_list, june_trj_list)) # 7
```

Plot with trajectory and GO enrichment plots for each cluster 
```{r}
# ==========================================
# STEP 1: Prepare June Treatment Data
# ==========================================

# Individual trajectories comparing Ambient vs Heat for June
trajectory_long <- june_trajectories %>%
  left_join(features %>% dplyr::select(pair_id), by = "pair_id") %>%
  # Pivot the specific treatment columns
  pivot_longer(cols = c(Ambient, Heat), 
               names_to = "treatment", 
               values_to = "cor") %>%
  # Ensure Ambient is the baseline on the left of the plot
  mutate(treatment = factor(treatment, levels = c("Ambient", "Heat")))

# Cluster means and SE for the two treatments
cluster_stats <- trajectory_long %>%
  dplyr::select(pair_id, miRNA, mRNA, timepoint, cluster, treatment, cor) %>%
  group_by(cluster, treatment) %>%
  summarise(
    mean_cor = mean(cor, na.rm = TRUE),
    se_cor = sd(cor, na.rm = TRUE) / sqrt(n()),
...
# ==========================================
# STEP 3: Create trajectory plots (Ambient vs Heat)
# ==========================================
...
trajectory_plots <- lapply(all_clusters, function(clust) {
  ...
  ggplot() +
    ...
    # Individual reaction norms (faint lines connecting Amb to Heat)
    geom_line(data = traj_data, 
              aes(x = treatment, y = cor, group = pair_id), 
              alpha = 0.08, color = "grey60", linewidth = 0.3) +
    
    # Mean ± SE Ribbon (Note: group=1 is required for ribbons/lines with factors)
    geom_ribbon(data = stat_data, 
                aes(x = treatment, ymin = mean_cor - se_cor, 
                    ymax = mean_cor + se_cor, group = 1), 
                fill = "red3", alpha = 0.2) +
    
    # Mean Trajectory Line
    geom_line(data = stat_data, 
              aes(x = treatment, y = mean_cor, group = 1), 
              color = "red3", linewidth = 1.2) +
    ...
    labs(
      title = paste0("June Cluster ", clust), 
      x = "Treatment (June)", 
      y = "Spearman correlation"
    ) +
...
# ==========================================
# STEP 4: Create GO enrichment plots for each cluster (Sized by Fold Enrichment)
# ==========================================
top_n_terms <- 10  

go_data_plot <- june_trt_functions %>%
  # Filter for BP if you have other ontologies in the file
  filter(ontology == "Biological Processes") %>%
  # Ensure numeric variables for calculation & sorting
  mutate(
    Fisher      = as.numeric(Fisher),
    Significant = as.numeric(Significant),
    Expected    = as.numeric(Expected),
    # Calculate Fold Enrichment (Enrichment Magnitude)
  ...
  ungroup() %>%
  mutate(
    # Clean up term names for the y-axis
    Term_short   = str_trunc(Term, width = 50, side = "right"),
    # Calculate -log10 p-value
    neglog10p    = -log10(Fisher),
    # Ensure terms are ordered by significance within each cluster facet
    Term_ordered = reorder_within(Term_short, neglog10p, cluster)
  )

# Calculate global enrichment range across all clusters for a unified size scale
enrich_limits <- range(go_data_plot$Enrichment, na.rm = TRUE)

go_plots <- lapply(all_clusters, function(clust) {
  ...
  ggplot(go_data, aes(x = neglog10p, y = Term_ordered, size = Enrichment)) + 
    geom_point(alpha = 0.8, color = "black") + 
    # Use tidytext's scale_y_reordered to handle the reorder_within
    scale_y_reordered() +
    scale_size_continuous(
      limits = enrich_limits,
      range  = c(3, 8),
      breaks = scales::pretty_breaks(n = 4) # Automatically generates clean break numbers
    ) +
    labs(
      x = expression(-log[10](italic(P))), 
      y = "", 
      size = "Fold Enrichment",
      title = paste("June Cluster", clust, "Functions")
    ) +
    theme_bw(base_size = 12) +
    theme(
...
# Save
ggsave("../output/Molecular/interactions/networks/june_trt_cluster_trajectory_functional.pdf", 
       combined_plot, 
       width = 16, height = 4 * n_clusters, 
       limitsize = FALSE)
ggsave("../output/Molecular/interactions/networks/june_trt_cluster_trajectory_functional.png", 
       combined_plot, 
       width = 16, height = 4 * n_clusters, 
       limitsize = FALSE)
```
...
### August ambient v. heat 

Filter correlation data for August ambient and heat samples 
```{r}
aug_trt_all_correlations_high_miRNA <- all_correlations_high_miRNA %>%
  filter(timepoint %in% c("TP7")) %>%
  filter(treatment %in% c("Ambient", "Heat"))

aug_summary <- aug_trt_all_correlations_high_miRNA %>%
  group_by(timepoint, treatment) %>%
  summarise(
    unique_miRNAs = n_distinct(miRNA),
    unique_mRNAs = n_distinct(mRNA),
...
Reshape df for trajectory calculations
```{r}
aug_trajectories <- aug_trt_all_correlations_high_miRNA %>%
  dplyr::select(pair_id, miRNA, mRNA, timepoint, treatment, cor_value) %>%
  dplyr::group_by(pair_id, miRNA, mRNA, timepoint, treatment) %>%
  dplyr::summarise(cor_value = mean(cor_value, na.rm = TRUE), .groups = "drop") %>%
  # Drop time_num here, keep only what's needed for pivot
...
aug_pair_fits <- aug_trajectories %>%
  rowwise() %>%
  do(fit_pair_trajectory(.)) %>%
  ungroup()
```
...
cluster_summary <- aug_trajectories %>%
  group_by(cluster) %>%
  summarise(unique_pairs = n_distinct(pair_id),
            unique_miRNAs = n_distinct(miRNA),
            unique_mRNAs = n_distinct(mRNA))
...
Save trajectory info
```{r}
write.csv(aug_trajectories, "../output/Molecular/interactions/networks/aug_trt_cluster_trajectory.csv")
```

Plot trajectories 
```{r}
# Prepare long format data (same as before)
aug_treatment_long <- aug_trajectories %>%
  left_join(features %>% dplyr::select(pair_id, cluster), by = c("pair_id", "cluster")) %>%
  # Pivot Ambient and Heat into one column
  pivot_longer(cols = c(Ambient, Heat), 
               names_to = "treatment", 
...
# Calculate Stats (Mean ± SE) per cluster and treatment
cluster_stats <- aug_treatment_long %>%
  group_by(cluster, treatment) %>%
  summarise(
    mean_cor = mean(cor, na.rm = TRUE),
    se_cor = sd(cor, na.rm = TRUE) / sqrt(n()),
...
p_faceted <- ggplot() +
  # Faint individual reaction norms (slopes) for each pair
  geom_line(data = aug_treatment_long, 
            aes(x = treatment, y = cor, group = pair_id), 
            alpha = 0.08, color = "grey60", linewidth = 0.4) +
  ...
  labs(x = "Treatment", 
       y = "Correlation (miRNA-mRNA)",
       title = "Thermal Response of miRNA-mRNA Pairs by Cluster",
       subtitle = "August Timepoint: Ambient vs. Heat Stress") +
  theme_bw(base_size = 12) +
  theme(
    strip.text = element_text(face = "bold", size = 11),
    panel.grid.minor = element_blank(),
...
## Enrichment of August treatment clusters 
...
### Cluster 1 - August treatment 

Subset by Cluster 1 and select only negative correlations for Ambient and positive correlations for Heat 
```{r}
subset_cluster1 <- aug_trajectories %>%
  dplyr::filter(cluster == "1", Ambient < 0, Heat > 0)
...
#### Biological Processes - cluster 1 August treatment
...
#### Molecular Functions - cluster 1 August treatment
...
```{r}
cluster1_aug_trt_functional <- rbind(GO_BP_En_sig, GO_MF_En_sig)

# Save as csv 
write.csv(cluster1_aug_trt_functional, "../output/Molecular/interactions/networks/aug_trt_cluster1_functional.csv")
```

### Cluster 2 - August treatment 

Subset by Cluster 2 and select only negative correlations for Heat and positive correlations for Ambient 
```{r}
subset_cluster2 <- aug_trajectories %>%
  dplyr::filter(cluster == "2", Heat < 0, Ambient > 0)
...
#### Biological Processes - cluster 2 August treatment
...
#### Molecular Functions - cluster 2 August treatment
...
```{r}
cluster2_aug_trt_functional <- rbind(GO_BP_En_sig, GO_MF_En_sig)

# Save as csv 
write.csv(cluster2_aug_trt_functional, "../output/Molecular/interactions/networks/aug_trt_cluster2_functional.csv")
```

### Cluster 3 - August treatment 

Subset by Cluster 3 and select only negative correlations for both Ambient and Heat
```{r}
subset_cluster3 <- aug_trajectories %>%
  dplyr::filter(cluster == "3", Heat < 0, Ambient < 0)
...
Generate results table 
```{r}
GO_BP_En <- GenTable(GO_BP, Fisher = GO_BP_FE, orderBy = "Fisher", numChar = 51, topNodes = 200)
```
...
```{r}
cluster3_aug_trt_functional <- rbind(GO_BP_En_sig, GO_MF_En_sig)

# Save as csv 
write.csv(cluster3_aug_trt_functional, "../output/Molecular/interactions/networks/aug_trt_cluster3_functional.csv")
```

## Plot August treatment functional results 

Bind functional results from all clusters 
```{r}
aug_trt_functions <- rbind(cluster1_aug_trt_functional, cluster2_aug_trt_functional, cluster3_aug_trt_functional)

# Save as csv 
write.csv(aug_trt_functions, "../output/Molecular/interactions/networks/aug_trt_clusters_functional.csv")
```

Read in functional info if needed for plotting 
```{r}
aug_trt_functions <- read.csv("../output/Molecular/interactions/networks/aug_trt_clusters_functional.csv")
```

Plot with trajectory and GO enrichment plots for each cluster 
```{r}
# ==========================================
# STEP 1: Prepare August Treatment Data
# ==========================================

# Individual trajectories comparing Ambient vs Heat for June
trajectory_long <- aug_trajectories %>%
  left_join(features %>% dplyr::select(pair_id), by = "pair_id") %>%
  # Pivot the specific treatment columns
  pivot_longer(cols = c(Ambient, Heat), 
               names_to = "treatment", 
...
trajectory_plots <- lapply(all_clusters, function(clust) {
  ...
  ggplot() +
    ...
    labs(
      title = paste0("August Cluster ", clust), 
      x = "Treatment (August)", 
      y = "Spearman correlation"
    ) +
...
# ==========================================
# STEP 4: Create GO enrichment plots with Inf/Floor handling
# ==========================================
top_n_terms <- 10  

go_data_plot <- aug_trt_functions %>%
  filter(ontology == "Biological Processes") %>%
  mutate(
    Fisher      = as.numeric(Fisher),
    Significant = as.numeric(Significant),
    Expected    = as.numeric(Expected),
    # Prevent division by zero by setting a small lower bound for Expected
    Expected_safe = ifelse(Expected == 0, 0.01, Expected),
    # Calculate Fold Enrichment with safe expected values
    Enrichment_raw = Significant / Expected_safe,
    # Cap extreme outliers at 50 so dot scales remain proportional
    Enrichment  = pmin(Enrichment_raw, 50)
  ) %>%
  group_by(cluster) %>%
  arrange(Fisher) %>%
...
# Safely extract non-infinite range for global size scaling
enrich_limits <- range(go_data_plot$Enrichment[is.finite(go_data_plot$Enrichment)], na.rm = TRUE)
...
go_plots <- lapply(all_clusters, function(clust) {
  ...
  ggplot(go_data, aes(x = neglog10p, y = Term_ordered, size = Enrichment)) + 
    ...
    labs(
      ...
      y = "", 
      size = "Fold Enrichment",
      title = paste("August Cluster", clust, "Functions")
    ) +
    theme_bw(base_size = 12) +
    theme(
      plot.title         = element_text(face = "bold", size = 14, hjust = 0.5),
...
# Save
ggsave("../output/Molecular/interactions/networks/aug_trt_cluster_trajectory_functional.pdf", 
       combined_plot, 
       width = 16, height = 4 * n_clusters, 
       limitsize = FALSE)
ggsave("../output/Molecular/interactions/networks/aug_trt_cluster_trajectory_functional.png", 
       combined_plot, 
       width = 16, height = 4 * n_clusters, 
       limitsize = FALSE)
```