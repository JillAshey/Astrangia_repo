...
pair_trajectories <- seasonal_all_correlations_high_miRNA %>%
  dplyr::select(pair_id, miRNA, mRNA, timepoint, cor_value) %>%
...
features <- pair_fits %>%
  ...
  dplyr::select(pair_id, linear, quadratic)
...
pair_trajectories <- pair_trajectories %>%
  left_join(features %>% dplyr::select(pair_id, raw_cluster), by = "pair_id") %>%
...
Read back in if needed for plotting 
```{r}
pair_trajectories <- read.csv("../output/Molecular/interactions/networks/seasonal_cluster_trajectory.csv")
```
...
# ==========================================
# STEP 5: Combine plots using patchwork with A, B, C tags
# ==========================================
n_clusters <- length(unique(cluster_stats$cluster))
# 1. Collect all individual plots in a flat list: 
# [Clust1_traj, Clust1_go, Clust2_traj, Clust2_go, ...]
flat_plot_list <- list()

clusters <- sort(unique(cluster_stats$cluster))

for (clust in clusters) {
  # Get cluster index for trajectory_plots
  clust_idx <- which(clusters == clust)
  
  flat_plot_list <- c(
    flat_plot_list, 
    list(trajectory_plots[[clust_idx]], go_plots[[as.character(clust)]])
  )
}

# 2. Wrap all plots into a 2-column grid and add auto-tagging
combined_plot <- wrap_plots(flat_plot_list, ncol = 2, widths = c(1, 1.2)) +
  plot_annotation(
    tag_levels = 'A' # Adds A, B, C, D, E, F... left-to-right, top-to-bottom
  ) &
  theme(
    plot.tag = element_text(size = 14, face = "bold") # Format panel tags
  )
