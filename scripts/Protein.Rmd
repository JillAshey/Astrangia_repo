...
prot_summary <- prot_exp_filtered %>%
  ...
  dplyr::summarize(
    ...
    sem_prot_offset = mean_prot + sem_prot + 0.2,
    .groups = "drop"
  )

prot_time_summary <- prot_exp_filtered %>%
  group_by(Month) %>%
  dplyr::summarize(
    n = n(),  # Count observations per group
    mean_prot = mean(prot_mg.cm2, na.rm = TRUE),
...
month_letters_manual <- tibble::tibble(
  ...
  Letters = c("a", "a", "a", "a", "a", "a", "b", "a"),
  y_position = c(2.32, 2.97, 1.92, 2.04, 3.32, 2.40, 0.76, 1.98)) # 0.2 offset from highest sd 
```
...
ggsave("../output/Physiology/SolubleProtein/SolubleProtein_Treatment_Timepoin_dotwhisker_sem.pdf", prot_dot_sem, width = 22, height = 12)
ggsave("../output/Physiology/SolubleProtein/SolubleProtein_Treatment_Timepoin_dotwhisker_sem.png", prot_dot_sem, width = 22, height = 12)
