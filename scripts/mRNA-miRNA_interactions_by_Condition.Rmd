...
# Save as csv 
write.csv(comparison_df_true_pcc_degs_annot_inf, "../output/Molecular/interactions/miRNA_mRNA_TP0_v_TP5_heat_pcc_lfc_annot.png")
```
...
# Save as csv 
write.csv(comparison_df_degs_annot, "../output/Molecular/interactions/miRNA_mRNA_TP5_amb_v_TP5_heat_pcc_lfc_annot.png")
```
...
# Save as csv 
write.csv(comparison_df_true_pcc_degs_annot_inf, "../output/Molecular/interactions/miRNA_mRNA_TP0_v_TP7_amb_pcc_lfc_annot.png")
```
...
comparison_df <- tp0_unique %>%
  ...
  inner_join(tp7_heat_unique %>% dplyr::select(miRNA, mRNA, PCC.cor_tp7_heat = PCC.cor), by = c("miRNA", "mRNA"))
```
...
comparison_df <- comparison_df %>%
  ...
  filter(sign_switch == "TRUE") %>%
  filter(abs(PCC.cor_tp0) > 0.7) %>%
  filter(abs(PCC.cor_tp7_heat) > 0.7) 
```
...
This represents the mRNA-miRNA pairs that have an abs(PCC.cor) > 0.7, switched correlation direction from tp0 to tp7 heat, and are differentially expressed between tp0 and tp7 heat. 
...
# Save as csv 
write.csv(comparison_df_degs_annot, "../output/Molecular/interactions/miRNA_mRNA_TP0_v_TP7_heat_pcc_lfc_annot.png")

blah <- comparison_df_degs_annot %>%
  filter(miRNA_repression_inferred == "miRNA repression of mRNA at TP7 heat")
...
# 2. Define color functions
pcc_col_fun <- colorRamp2(c(-1, 0, 1), c("#762a83", "white", "#1b7837"))
lfc_col_fun <- colorRamp2(c(-4, 0, 5), c("blue", "white", "red"))
...
comparison_df <- tp7_amb_unique %>%
  dplyr::select(miRNA, mRNA, PCC.cor_tp7_amb = PCC.cor) %>%
  inner_join(tp7_heat_unique %>% dplyr::select(miRNA, mRNA, PCC.cor_tp7_heat = PCC.cor), by = c("miRNA", "mRNA"))
```
