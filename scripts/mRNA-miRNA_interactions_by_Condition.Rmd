...
filt_outrm_poa <- genefilter((mrna_subset), ffun) #apply filter

mrna_subset_filt <- mrna_subset[filt_outrm_poa,] #keep only rows that passed filter
dim(mrna_subset)
dim(mrna_subset_filt)
```
...
mRNA_norm <- normalize_counts(mrna_subset_filt)
...
length(unique(tp5_amb$miRNA))
length(unique(tp5_amb$mRNA))
```
...
# Prepare edges for TP7, with color and width based on PCC
edges_tp7 <- comparison_df_true_pcc_degs_annot_inf %>%
  select(miRNA, mRNA, PCC.cor_tp7_amb) %>%
  rename(from = miRNA, to = mRNA) %>%
  mutate(
    edge_color = ifelse(PCC.cor_tp7_amb > 0, "red", "blue"),
    edge_width = abs(PCC.cor_tp7_amb)  # scale factor for thickness
  )

# Build igraph object
graph_tp7 <- graph_from_data_frame(d = edges_tp7, vertices = nodes, directed = FALSE)

# Plot the network with node color and edge width/color
p_tp7 <- ggraph(graph_tp7, layout = "kk") +
  geom_edge_link(aes(color = edge_color, width = edge_width), alpha = 0.7) +
  geom_node_point(aes(color = type), size = 5) +
  #geom_node_text(aes(label = name), repel = TRUE, size = 3) +
  scale_edge_color_manual(
 ...
 #       plot.background = element_blank()) +
  #labs(title = "miRNA–mRNA Interaction Network at TP5") +
  theme(legend.position = "right"); p_tp7

# Plot togerher and save
combined_plot <- p_tp0 + p_tp7
