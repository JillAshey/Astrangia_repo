...
comparison_df_degs_annot <- comparison_df_degs_annot %>%
  mutate(miRNA_repression_inferred = case_when(
    PCC.cor_tp0 >  0 & PCC.cor_tp7_amb <  0 & log2FoldChange > 0  ~ "miRNA repression at TP7 ambient",
    PCC.cor_tp0 <  0 & PCC.cor_tp7_amb >  0 & log2FoldChange < 0  ~ "miRNA repression at TP0",
    PCC.cor_tp0 >  0 & PCC.cor_tp7_amb <  0 & log2FoldChange < 0  ~ "High mRNA expression at TP7 ambient, regardless of repression",
    PCC.cor_tp0 <  0 & PCC.cor_tp7_amb >  0 & log2FoldChange > 0  ~ "High mRNA expression at TP0, regardless of repression",
...
# 2. Define color functions
pcc_col_fun <- colorRamp2(c(-1, 0, 1), c("blue", "white", "red"))
lfc_col_fun <- colorRamp2(c(-5, 0, 5), c("purple", "white", "orange"))

# 3. Row annotation: repression inference
row_annot <- rowAnnotation(
  Inference = comparison_df_degs_annot$miRNA_repression_inferred,
  col = list(Inference = c(
    "miRNA repression at TP7 ambient" = "#7F58AF",
    "miRNA repression at TP0" = "#64C5EB",
    "High mRNA expression at TP7 ambient, regardless of repression" = "#E84D8A",
    "High mRNA expression at TP0, regardless of repression" = "#FEB326",
    "no inference" = "grey85"
  )),
  show_annotation_name = TRUE
)
