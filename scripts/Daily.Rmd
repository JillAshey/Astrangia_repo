...
par(mfrow=c(2,3))
plot(SW.chem$Tank, SW.chem$Temperature, xlab="Tank", ylab="Temperature°C", ylim = c(3.5,15), las = 2)
plot(SW.chem$Tank, SW.chem$Salinity, xlab="Tank", ylab="Salinity psu", ylim = c(30,34), las = 2)
plot(SW.chem$Tank, SW.chem$pH.Total, xlab="Tank", ylab="pH Total Scale", ylim = c(8,8.3), las = 2)
plot(SW.chem$Tank, SW.chem$Light, xlab="Tank", ylab="Light µmol photons m^2 s^-1", ylim=c(65,150),las=2)
plot(SW.chem$Tank, SW.chem$Flow_mL_sec, xlab="Tank", ylab="Flow mL/sec", ylim=c(2,20),las=2)
dev.off()
...
```{r}
mod.temp <- aov(Temperature ~ Tank, data = SW.chem)
mod.temp
summary(mod.temp)
hist(mod.temp$residuals)

mod.sal <- aov(Salinity ~ Tank, data = SW.chem)
mod.sal
summary(mod.sal)
...
mod.flow <- aov(Flow_mL_sec ~ Tank, data = SW.chem)
mod.flow
summary(mod.flow)
