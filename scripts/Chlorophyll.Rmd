...
chl_time_summary <- chl_exp_filtered %>%
  group_by(Month) %>%
  dplyr::summarize(
    n = n(),  # Count observations per group
    mean_chla = mean(chla.ug.cm2, na.rm = TRUE),
    sd_chla = sd(chla.ug.cm2, na.rm = TRUE),
    sem_chla = sd_chla / sqrt(n),  # Correct SEM formula
    sem_chla_offset = mean_chla + sem_chla + 0.2,
    .groups = "drop"
  )
```
...
month_letters_manual <- tibble::tibble(
  ...
  y_position = c(1.31, 1.39, 1.79, 1.96, 1.96, 1.63, 1.37, 2.29)) # 0.2 offset from highest sem
```
...
ggsave("../output/Physiology/Chlorophyll/Chla_Treatment_Timepoint_dotwhisker_sem.pdf", chla_dot_sem, width = 22, height = 12)
ggsave("../output/Physiology/Chlorophyll/Chla_Treatment_Timepoint_dotwhisker_sem.png", chla_dot_sem, width = 22, height = 12)
