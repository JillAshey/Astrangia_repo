...
# Read in xml info
xmlfile <- xmlParse("http://131.128.103.229:80/cgi-bin/datalog.xml?sdate=210109&days=4") #read in the date plus x days of Apex data
...
write.csv(Probe.Data, "~/Desktop/20210112_Apex_Data_Output.data.csv") #write file to save data

# Plot Temp and save to output
# CHANGE DATE FOR NEW PDF (risk overwritting previous)
pdf("~/Desktop/20210112_Apex_Plot_Output.data.pdf")
#par(mfrow=c(2,1))
plot(as.numeric(as.character(Temp1)) ~ Date.Time, Probe.Data, col = "grey", lwd = 2, type="l", ylim=c(6,12),  xlab="Time", ylab="Temperature °C")
