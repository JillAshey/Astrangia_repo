...
I am left with 120 miRNAs. Merge the Treatment and Timepoint columns together into one column called 'Condition'. Then run DESeq2 so the design is ~Condition
...
sum(TP0_v_5_amb$padj <0.05, na.rm=T) # 20 miRNA with significant p-values
TP0_v_5_amb.sig <- subset(TP0_v_5_amb, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP0_v_5_amb.sig$log2FoldChange <0, na.rm=T) # 13 miRNAs are downregulated in TP5 ambient relative to TP0 ambient
sum(TP0_v_5_amb.sig$log2FoldChange >0, na.rm=T) # 7 miRNAs are upregulated in TP5 ambient relative to TP0 ambient
...
# ggsave("../output/Molecular/smRNA/heatmap_TP0_amb_v_TP7_amb_DEM.pdf", heatmap, width = 8, height = 10)
...
sum(TP0_v_5_heat$padj <0.05, na.rm=T) # 32 miRNAs with significant p-values
TP0_v_5_heat.sig <- subset(TP0_v_5_heat, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP0_v_5_heat.sig$log2FoldChange <0, na.rm=T) # 20 miRNAs are downregulated in TP5 heat relative to TP0 ambient
sum(TP0_v_5_heat.sig$log2FoldChange >0, na.rm=T) # 12 miRNAs are upregulated in TP5 heat relative to TP0 ambient
...
ggsave("../output/Molecular/smRNA/heatmap_TP0_amb_v_TP5_heat_DEM.pdf", heatmap, width = 8, height = 10)
```
...
ggsave("../output/Molecular/smRNA/heatmap_TP0_amb_v_TP7_heat_DEM.pdf", heatmap, width = 8, height = 10)
```
...
sum(TP5_heat_v_7_heat$padj <0.05, na.rm=T) # 9 miRNAs with significant p-values
TP5_heat_v_7_heat.sig <- subset(TP5_heat_v_7_heat, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP5_heat_v_7_heat.sig$log2FoldChange <0, na.rm=T) # 9 miRNAs are downregulated in TP7 heat relative to TP5 heat
...
ggsave("../output/Molecular/smRNA/heatmap_TP5_heat_v_TP7_heat_DEM.pdf", heatmap, width = 8, height = 10)
```
...
```{r}
DEMs.all <- rbind(TP0_v_5_amb.DEMs, TP0_v_7_amb.DEMs, TP0_v_5_heat.DEMs, TP0_v_7_heat.DEMs, TP5_heat_v_7_heat.DEMs)
...
There are 50 unique differentially expressed miRNAs. Visualize the differentially expressed miRNAs. 
...
ggsave("../output/Molecular/smRNA/heatmap_DEMs.pdf", heatmap, width = 8, height = 10)
ggsave("../output/Molecular/smRNA/heatmap_DEMs.png", heatmap, width = 8, height = 10)
```
