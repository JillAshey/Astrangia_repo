...
Use DESeq2 to evaluate differential expression of miRNAs. 
...
```{r}
counts <- read_csv("../data/Molecular/smRNA/miRNA_count_info.csv") %>%
  dplyr::select(-c("...1", "miRDeep2.score", "estimated_true_positive", "rfam.alert", "total.read.count", "mature.read.count", "loop.read.count", "star.read.count", "significant.randfold.p.value", "consensus.mature.sequence", "consensus.star.sequence", "consensus.precursor.sequence", "precursor.coordinate", "Type"))
counts <- as.data.frame(counts)
dim(counts)
length(unique(counts$ID))
...
```{r}
colnames(counts) <- sub("read_count_", "", colnames(counts))
```
...
```{r}
ffun<-filterfun(pOverA(0.15,10))  #set up filtering parameters--LOOK INTO FILTERING PARAMETERS
...
I am left with 226 miRNAs. Merge the Treatment and Timepoint columns together into one column called 'Condition'. Then run DESeq2 so the design is ~Condition
...
sum(TP0_v_5_amb$padj <0.05, na.rm=T) # 11 miRNA with significant p-values
TP0_v_5_amb.sig <- subset(TP0_v_5_amb, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP0_v_5_amb.sig$log2FoldChange <0, na.rm=T) # 9 miRNAs are downregulated in TP5 ambient relative to TP0 ambient
sum(TP0_v_5_amb.sig$log2FoldChange >0, na.rm=T) # 2 miRNAs are upregulated in TP5 ambient relative to TP0 ambient
...
#write.csv(counts(TP0_v_5_amb.sig.list), file = "../output/Molecular/smRNA/TP0_amb_v_TP5_amb_DEM.csv")
...
## Save plots 
ggsave("../output/Molecular/smRNA/ma_plot_TP0_amb_v_TP5_amb_DEM.pdf", ma_plot, width = 10, height = 8)
ggsave("../output/Molecular/smRNA/pca_plot_TP0_amb_v_TP5_amb_DEM.pdf", pca_plot, width = 10, height = 8)
pdf("../output/Molecular/smRNA/heatmap_TP0_amb_v_TP5_amb_DEM.pdf", width = 8, height = 10)
print(heatmap)
dev.off()
...
write.csv(TP0_v_5_amb.sig.list.full, file = "../output/Molecular/smRNA/TP0_amb_v_TP5_amb_DEM.csv")
```
...
sum(TP0_v_7_amb$padj <0.05, na.rm=T) # 25 miRNAs with significant p-values
TP0_v_7_amb.sig <- subset(TP0_v_7_amb, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP0_v_7_amb.sig$log2FoldChange <0, na.rm=T) # 14 miRNAs are downregulated in TP5 ambient relative to TP0 ambient
sum(TP0_v_7_amb.sig$log2FoldChange >0, na.rm=T) # 11 miRNAs are upregulated in TP5 ambient relative to TP0 ambient
...
#write.csv(counts(TP0_v_7_amb.sig.list), file = "../output/Molecular/smRNA/TP0_amb_v_TP7_amb_DEM.csv")
...
## Save plots 
ggsave("../output/Molecular/smRNA/ma_plot_TP0_amb_v_TP7_amb_DEM.pdf", ma_plot, width = 10, height = 8)
ggsave("../output/Molecular/smRNA/pca_plot_TP0_amb_v_TP7_amb_DEM.pdf", pca_plot, width = 10, height = 8)
pdf("../output/Molecular/smRNA/heatmap_TP0_amb_v_TP7_amb_DEM.pdf", width = 8, height = 10)
print(heatmap)
dev.off()
...
write.csv(TP0_v_7_amb.sig.list.full, file = "../output/Molecular/smRNA/TP0_amb_v_TP7_amb_DEM.csv")
```
...
sum(TP0_v_5_heat$padj <0.05, na.rm=T) # 13 miRNAs with significant p-values
TP0_v_5_heat.sig <- subset(TP0_v_5_heat, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP0_v_5_heat.sig$log2FoldChange <0, na.rm=T) # 8 miRNAs are downregulated in TP5 heat relative to TP0 ambient
sum(TP0_v_5_heat.sig$log2FoldChange >0, na.rm=T) # 5 miRNAs are upregulated in TP5 heat relative to TP0 ambient
...
#write.csv(counts(TP0_v_5_heat.sig.list), file = "../output/Molecular/smRNA/TP0_amb_v_TP5_heat_DEM.csv")
...
## Save plots 
ggsave("../output/Molecular/smRNA/ma_plot_TP0_amb_v_TP5_heat_DEM.pdf", ma_plot, width = 10, height = 8)
ggsave("../output/Molecular/smRNA/pca_plot_TP0_amb_v_TP5_heat_DEM.pdf", pca_plot, width = 10, height = 8)
pdf("../output/Molecular/smRNA/heatmap_TP0_amb_v_TP5_heat_DEM.pdf", width = 8, height = 10)
print(heatmap)
dev.off()
...
write.csv(TP0_v_5_heat.sig.list.full, file = "../output/Molecular/smRNA/TP0_amb_v_TP5_heat_DEM.csv")
```
...
sum(TP0_v_7_heat$padj <0.05, na.rm=T) # 29 miRNAs with significant p-values
TP0_v_7_heat.sig <- subset(TP0_v_7_heat, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP0_v_7_heat.sig$log2FoldChange <0, na.rm=T) # 18 miRNAs are downregulated in TP7 heat relative to TP0 ambient
sum(TP0_v_7_heat.sig$log2FoldChange >0, na.rm=T) # 11 miRNAs are upregulated in TP7 heat relative to TP0 ambient
...
write.csv(counts(TP0_v_7_heat.sig.list), file = "../output/Molecular/smRNA/TP0_amb_v_TP7_heat_DEM.csv")
...
## Save plots 
ggsave("../output/Molecular/smRNA/ma_plot_TP0_amb_v_TP7_heat_DEM.pdf", ma_plot, width = 10, height = 8)
ggsave("../output/Molecular/smRNA/pca_plot_TP0_amb_v_TP7_heat_DEM.pdf", pca_plot, width = 10, height = 8)
pdf("../output/Molecular/smRNA/heatmap_TP0_amb_v_TP7_heat_DEM.pdf", width = 8, height = 10)
print(heatmap)
dev.off()
...
write.csv(TP0_v_7_heat.sig.list.full, file = "../output/Molecular/smRNA/TP0_amb_v_TP7_heat_DEM.csv")
```
...
sum(TP5_heat_v_7_heat$padj <0.05, na.rm=T) # 6 miRNAs with significant p-values
TP5_heat_v_7_heat.sig <- subset(TP5_heat_v_7_heat, padj<0.05,) #identify signficant pvalues with 5%FDR
sum(TP5_heat_v_7_heat.sig$log2FoldChange <0, na.rm=T) # 6 miRNAs are downregulated in TP7 heat relative to TP5 heat
sum(TP5_heat_v_7_heat.sig$log2FoldChange >0, na.rm=T) # 0 miRNAs are upregulated in TP7 heat relative to TP5 heat
...
write.csv(counts(TP5_heat_v_7_heat.sig.list), file = "../output/Molecular/smRNA/TP5_heat_v_TP7_heat_DEM.csv")
...
## Save plots 
ggsave("../output/Molecular/smRNA/ma_plot_TP5_heat_v_TP7_heat_DEM.pdf", ma_plot, width = 10, height = 8)
ggsave("../output/Molecular/smRNA/pca_plot_TP5_heat_v_TP7_heat_DEM.pdf", pca_plot, width = 10, height = 8)
pdf("../output/Molecular/smRNA/heatmap_TP5_heat_v_TP7_heat_DEM.pdf", width = 8, height = 10)
print(heatmap)
dev.off()
...
write.csv(TP5_heat_v_7_heat.sig.list.full, file = "../output/Molecular/smRNA/TP5_heat_v_TP7_heat_DEM.csv")
```
...
sum(TP5_amb_v_TP5_heat$padj <0.05, na.rm=T) # 0 miRNAs with significant p-values
```
...
sum(TP7_amb_v_TP7_heat$padj <0.05, na.rm=T) # 0 miRNAs with significant p-values
```
...
sum(TP5_heat_v_TP7_amb$padj <0.05, na.rm=T) # 0 miRNAs with significant p-values
```
...
TP5_heat_v_7_heat.DEMs <- as.data.frame(row.names(TP5_heat_v_7_heat.sig.list))
colnames(TP5_heat_v_7_heat.DEMs) <- "DEMs"
length(t(unique(TP5_heat_v_7_heat.DEMs)))
```
...
write.csv(counts(Unique.sig.list), file="../output/Molecular/smRNA/Unique_DEMs.csv")
```

There are 26 unique differentially expressed miRNAs. Visualize the differentially expressed miRNAs. 
...
Save DEM plots 
```{r}
ggsave("../output/Molecular/smRNA/pca_plot_DEMs.pdf", pca_plot, width = 10, height = 8)
ggsave("../output/Molecular/smRNA/pca_plot_DEMs.png", pca_plot, width = 10, height = 8)

pdf("../output/Molecular/smRNA/heatmap_DEMs.pdf", width = 8, height = 10)
print(heatmap)
dev.off()
```
...
```{r}
DEMs.list <- read.csv("../output/Molecular/smRNA/Unique_DEMs.csv")
length(unique(DEMs.list$X))

list <- as.character(DEMs.list$X)

writeLines(list, "../output/Molecular/smRNA/DEM_list.txt")
```
