...
fld <- read_csv(file = "../output/Physiology/PR/PR_field_rates.csv") %>%
  mutate(Treatment = str_remove(Treatment, "-.*$"))

all <- rbind(exp, fld)

# Select only light measurements and ambient/field 
photo <- all %>%
  filter(Light_Value > 0) %>%
  filter(Treatment %in% c("Ambient", "Field"))
```
...
photo <- photo %>% 
  mutate(Month = case_when(
    ...
    Timepoint == "TP6" ~ "July", 
    Timepoint == "TP7" ~ "August")) %>%
  filter(Timepoint %in% c("TP3", "TP4", "TP5", "TP6", "TP7")) # remove TPs we don't have field data for

photo$micromol.cm2.h <- as.numeric(photo$micromol.cm2.h)
```
...
photo_summary <- photo %>%
  ...
  dplyr::summarize(
    ...
    mean_photo = mean(micromol.cm2.h, na.rm = TRUE),
    sd_photo = sd(micromol.cm2.h, na.rm = TRUE),
    se_photo = sd_photo / sqrt(n),  # SEM formula
    .groups = "drop"
  )
```
...
photo_dot <- ggplot(photo_summary, aes(x = Month, y = mean_photo, color = Treatment)) +
  ...
  geom_point(size = 10, aes(fill = Treatment), 
             shape = 21, color = "black", stroke = 2) +  # Thicker point borders
  annotate("text", x = Inf, y = 1, label = "ns", 
         hjust = 1.1, vjust = -1.1, size = 8, fontface = "bold") +
  scale_color_manual(values = c("Field" = "orange", 
                               "Ambient" = "#377EB8")) +
  scale_fill_manual(values = c("Acclimation" = "orange", 
                              "Ambient" = "#377EB8")) +
  xlab("") +
  ylab(expression(bold('Net Photosynthesis ('*mu~ 'mol' ~cm^-2*~ h^-1*')'))) +
  theme_classic() +  # Increased base font size
...
```{r}
# Histogram
ggplot(photo, aes(x = micromol.cm2.h)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()

# Q-Q Plot
qqnorm(photo$micromol.cm2.h)
qqline(photo$micromol.cm2.h, col = "red")

# Shapiro-Wilk Test
shapiro.test(photo$micromol.cm2.h)
```

Data is not normal, perform data transformation
```{r}
# log
photo$photo_log <- log(photo$micromol.cm2.h)
shapiro.test(photo$photo_log)
qqnorm(photo$photo_log)
qqline(photo$photo_log, col = "red")
ggplot(photo, aes(x = photo_log)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()
```
...
photo$Treatment <- as.factor(photo$Treatment)
photo$Timepoint <- as.factor(photo$Timepoint)

# Run two-way ANOVA
anova_result <- aov(photo_log ~ Treatment * Timepoint, data = photo)
summary(anova_result)

# Check for homogenity of variance 
leveneTest(photo_log ~ Treatment * Timepoint, data = photo)
...
resp$micromol.cm2.h <- as.numeric(resp$micromol.cm2.h)
```

Remove outliers using IQR method
```{r}
# Look for outliers visually
ggplot(resp, aes(x = Treatment, y = micromol.cm2.h)) + 
  geom_boxplot(outlier.color = "red", outlier.shape = 16) +
  theme_minimal()

# Calculate IQR
Q1 <- quantile(resp$micromol.cm2.h, 0.25, na.rm = T)
Q3 <- quantile(resp$micromol.cm2.h, 0.75, na.rm = T)
IQR_val <- Q3 - Q1
...
# Filter dataset to remove outliers
resp_exp_filtered <- resp[resp$micromol.cm2.h >= lower_bound & resp$micromol.cm2.h <= upper_bound, ]
...
resp_dot <- ggplot(resp_summary, aes(x = Month, y = mean_resp, color = Treatment)) +
  ...
  geom_point(size = 10, aes(fill = Treatment), 
             shape = 21, color = "black", stroke = 2) +  # Thicker point borders
    annotate("text", x = Inf, y = -0.5, label = "Time point p=0.006", 
         hjust = 1.1, vjust = -1.1, size = 8, fontface = "bold") +
  scale_color_manual(values = c("Field" = "orange", 
                               "Ambient" = "#377EB8")) +
  scale_fill_manual(values = c("Acclimation" = "orange", 
...
# Keep only field and ambient data 
chl_exp <- chl %>%
  filter(Treatment %in% c("Field", "Ambient"))
```
...
```{r}
# Histogram
ggplot(chl_exp, aes(x = chla.ug.cm2)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()

# Q-Q Plot
qqnorm(chl_exp$chla.ug.cm2)
qqline(chl_exp$chla.ug.cm2, col = "red")

# Shapiro-Wilk Test
shapiro.test(chl_exp$chla.ug.cm2)
```

Data is not normal, perform data transformation
```{r}
# sqrt 
chl_exp$chla_sqrt <- sqrt(chl_exp$chla.ug.cm2)
shapiro.test(chl_exp$chla_sqrt)
qqnorm(chl_exp$chla_sqrt)
qqline(chl_exp$chla_sqrt, col = "red")
ggplot(chl_exp, aes(x = chla_sqrt)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()

# log
chl_exp$chla_log <- log(chl_exp$chla.ug.cm2)
shapiro.test(chl_exp$chla_log)
qqnorm(chl_exp$chla_log)
qqline(chl_exp$chla_log, col = "red")
ggplot(chl_exp, aes(x = chla_log)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()
```
...
```{r}
# Ensure factors are properly set
chl_exp$Treatment <- as.factor(chl_exp$Treatment)
chl_exp$Timepoint <- as.factor(chl_exp$Timepoint)

# Run two-way ANOVA
anova_result <- aov(chla_sqrt ~ Treatment * Timepoint, data = chl_exp)
summary(anova_result)

# Check for homogenity of variance 
leveneTest(chla_sqrt ~ Treatment * Timepoint, data = chl_exp)
...
prot_dot <- ggplot(prot_summary, aes(x = Month, y = mean_prot, color = Treatment)) +
  ...
  geom_point(size = 10, aes(fill = Treatment), 
             shape = 21, color = "black", stroke = 2) +  # Thicker point borders
  annotate("text", x = Inf, y = 2.5, label = "Time point p<0.001", 
         hjust = 1.1, vjust = -1.1, size = 8, fontface = "bold") +
  scale_color_manual(values = c("Field" = "orange", 
                               "Ambient" = "#377EB8")) +
  scale_fill_manual(values = c("Acclimation" = "orange", 
...
```{r}
# Histogram
ggplot(prot_exp, aes(x = prot_mg.cm2)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()

# Q-Q Plot
qqnorm(prot_exp$prot_mg.cm2)
qqline(prot_exp$prot_mg.cm2, col = "red")

# Shapiro-Wilk Test
shapiro.test(prot_exp$prot_mg.cm2)
```

Data is not normal, perform data transformation
```{r}
# log
prot_exp$prot_log <- log(prot_exp$prot_mg.cm2)
shapiro.test(prot_exp$prot_log)
qqnorm(prot_exp$prot_log)
qqline(prot_exp$prot_log, col = "red")
ggplot(prot_exp, aes(x = prot_log)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()
```
...
```{r}
# Ensure factors are properly set
prot_exp$Treatment <- as.factor(prot_exp$Treatment)
prot_exp$Timepoint <- as.factor(prot_exp$Timepoint)

# Run two-way ANOVA
anova_result <- aov(prot_log ~ Treatment * Timepoint, data = prot_exp)
summary(anova_result)

# Check for homogenity of variance 
leveneTest(prot_log ~ Treatment * Timepoint, data = prot_exp)
...
Read in data
```{r}
carb <- read_csv("../output/Physiology/Carbohydrates/Carbohydrates.csv") %>%
  drop_na(carb_mg.cm2)
...
carb_dot <- ggplot(carb_summary, aes(x = Month, y = mean_carb, color = Treatment)) +
  ...
  geom_point(size = 10, aes(fill = Treatment), 
             shape = 21, color = "black", stroke = 2) +  # Thicker point borders
  annotate("text", x = 3.5, y = -0.1, label = "Time point p=0.029", 
         hjust = 1.1, vjust = -1.1, size = 8, fontface = "bold") +
  scale_color_manual(values = c("Field" = "orange", 
                               "Ambient" = "#377EB8")) +
  scale_fill_manual(values = c("Field" = "orange", 
                              "Ambient" = "#377EB8")) +
  xlab("") +
  ylab("Carbohydrates (mg/cm²)") +
  theme_classic() +  # Increased base font size
...
```{r}
# Histogram
ggplot(carb, aes(x = carb_mg.cm2)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()

# Q-Q Plot
qqnorm(carb$carb_mg.cm2)
qqline(carb$carb_mg.cm2, col = "red")

# Shapiro-Wilk Test
shapiro.test(carb$carb_mg.cm2)
```

Data is not normal, perform data transformation
```{r}
# log
carb$carb_log <- log(carb$carb_mg.cm2)
shapiro.test(carb$carb_log)
qqnorm(carb$carb_log)
qqline(carb$carb_log, col = "red")
ggplot(carb, aes(x = carb_log)) + 
  geom_histogram(bins = 30, fill = "skyblue", color = "black") + 
  theme_minimal()
```
...
carb$Timepoint <- as.factor(carb$Timepoint)

# Run two-way ANOVA
anova_result <- aov(carb_log ~ Treatment * Timepoint, data = carb)
summary(anova_result)

# Check for homogenity of variance 
leveneTest(carb_log ~ Treatment * Timepoint, data = carb)
...
Read in data and subset host and ambient/field
```{r}
biomass <- read.csv("../output/Physiology/AFDW/AFDW_biomass.csv")
...
host_dot <- ggplot(biomass_host_summary, aes(x = Month, y = mean_host, color = Treatment)) +
  ...
  geom_point(size = 10, aes(fill = Treatment), 
             shape = 21, color = "black", stroke = 2) +  # Thicker point borders
  annotate("text", x = Inf, y = 9, label = "Treatment p=0.052", 
         hjust = 1.1, vjust = -1.1, size = 8, fontface = "bold") +
  scale_color_manual(values = c("Field" = "orange", 
                               "Ambient" = "#377EB8")) +
  scale_fill_manual(values = c("Field" = "orange", 
...
# Run two-way ANOVA
anova_result <- aov(AFDW.mg.cm2 ~ Treatment * Timepoint, data = biomass_host_filtered)
summary(anova_result)

# Check for homogenity of variance 
leveneTest(AFDW.mg.cm2 ~ Treatment * Timepoint, data = biomass_host_filtered)
...
# 2. Apply global formatting
fld_phys_final <- phys_final +
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = 'A') & 
  # Use & to force a single legend and color mapping
  scale_color_manual(values = c("Field" = "orange", 
                               "Ambient" = "#377EB8"),
                     limits = c("Field", "Ambient")) &
  scale_fill_manual(values = c("Field" = "orange", 
                              "Ambient" = "#377EB8"),
                    limits = c("Field", "Ambient")) &
  theme(
...
# 3. Save with adjusted dimensions
ggsave("../output/Physiology/Physiology_field_panels.png", plot = fld_phys_final, 
       width = 40, height = 20, units = "in", dpi = 300)
ggsave("../output/Physiology/Physiology_field_panels.pdf", plot = fld_phys_final, 
       width = 40, height = 20, units = "in", dpi = 300)
```
