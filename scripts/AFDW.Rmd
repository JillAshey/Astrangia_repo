...
biomass_host_summary <- biomass_host_filtered %>%
  ...
  dplyr::summarize(
    ...
    sem_host_offset = mean_host + sem_host + 0.2,
    .groups = "drop"
  )
```
...
month_letters_manual <- tibble::tibble(
  ...
  y_position = c(7.21, 9.72, 7.81, 4.82, 8.32, 9.47, 8.10, 7.56))# 0.2 offset from highest sd 
```
...
ggsave("../output/Physiology/AFDW/AFDW_Treatment_Timepoint_dotwhisker_sem.pdf", host_dot_sem, width = 22, height = 12)
ggsave("../output/Physiology/AFDW/AFDW_Treatment_Timepoint_dotwhisker_sem.png", host_dot_sem, width = 22, height = 12)
...
# Keep field and ambient host samples 
metadata_fld <- metadata %>%
  filter(Treatment == c("Field", "Ambient"))
```

basically need to compare 
