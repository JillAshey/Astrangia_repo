...
# Read in xml info
xmlfile <- xmlParse("http://131.128.103.229:80/cgi-bin/datalog.xml?sdate=210308&days=14") #read in the date plus x days of Apex data
...
Apex.Data2 <- Apex.Data %>% select(c("date",
                                     ...
                                     "probe.name.21", "probe.value.21", # T_4
...
write.csv(Probe.Data, "20210322_Apex_Data_Output.data.csv") #write file to save data

# Plot Temp and save to output
# CHANGE DATE FOR NEW PDF (risk overwritting previous)
pdf("~/Desktop/PutnamLab/Repositories/Astrangia_repo/Astrangia_repo/output/Apex/20210322_Apex_Plot_Output.data.pdf")
#par(mfrow=c(2,1))
plot(as.numeric(as.character(Temp1)) ~ Date.Time, Probe.Data, col = "grey", lwd = 2, type="l", ylim=c(4,10),  xlab="Time", ylab="Temperature °C")
