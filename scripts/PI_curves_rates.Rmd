...
sample.info <- read_csv(file = "data/1_pi_curves/pi_curves_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/1_pi_curves/pi_curves_run_metadata.csv")
...
```{r, warning = FALSE}
df <- df %>%
  mutate(intervals = map2(data0, run.info, function(.x, .y) {
